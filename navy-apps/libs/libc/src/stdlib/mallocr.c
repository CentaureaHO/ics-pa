...
_PTR _DEFUN(_malloc_r, (ptr, nbytes), struct _reent* ptr _AND size_t nbytes) /* get a block */
{
    return malloc(nbytes);
}

void _DEFUN(_free_r, (ptr, aptr), struct _reent* ptr _AND _PTR aptr) { free(aptr); }

_PTR _DEFUN(_realloc_r, (ptr, ap, nbytes), struct _reent* ptr _AND _PTR ap _AND size_t nbytes)
{
...
static _VOID _DEFUN(_morecore_r, (ptr, nu),  /* ask system for more memory */
    struct _reent* ptr _AND register int nu) /* size index to get more of  */
{
    register mptr cp;
    ...
    /* Round up */
    if ((((size_t)cp) & ROUND_TO) != 0) _sbrk_r(ptr, (ROUND_TO + 1) - (((size_t)cp) & ROUND_TO));
    ...
    if ((cp = (mptr)_sbrk_r(ptr, 1 << rnu)) == (mptr)-1) return; /* no more room! */
...
_PTR _DEFUN(_malloc_r, (ptr, nbytes), struct _reent* ptr _AND size_t nbytes) /* get a block */
{
    ...
    if ((p = (unsigned char*)(ptr->_nextf[nunits])) == NULL) return (NULL);
...
#ifdef rcheck
    if (nbytes <= 0x10000) ((unsigned short*)p)[1] = (unsigned short)nbytes - 1;
    *((INT32*)(p + 4))          = MAGIC;
...
void _DEFUN(_free_r, (ptr, aptr), struct _reent* ptr _AND _PTR aptr)
{
    register int si;
    char*        ap = aptr;
...
_PTR _DEFUN(_realloc_r, (ptr, ap, nbytes), struct _reent* ptr _AND _PTR ap _AND size_t nbytes)
{
    ...
    if (ap == NULL) return (_malloc_r(ptr, nbytes));
#ifdef rcheck
    p = (char*)ap - ALIGN(8);
    if (p[1] < 13)
    ...
    onb = (1 << (p[1] + 3)) - OVERHEAD;
#endif
    if ((res = _malloc_r(ptr, nbytes)) == NULL) return (NULL);
...
void _DEFUN(_mstats_r, (ptr, s), struct _reent* ptr _AND char* s)
{
    register int  i, j;
    register mptr p;
