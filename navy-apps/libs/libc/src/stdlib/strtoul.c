...
unsigned long _DEFUN(
    _strtoul_r, (rptr, s, ptr, base), struct _reent* rptr _AND _CONST char* s _AND char** ptr _AND int base)
{
    ...
    if (base == 0 || base == 16)
    {
        ...
        {
            did_conversion = 1;
            if (base == 0) radix = 8; /* guess it's octal */
            s++;                      /* (but check for hex) */
            if (*s == 'X' || *s == 'x')
            {
                did_conversion = 0;
    ...
    if (ptr != NULL) *ptr = (char*)((did_conversion) ? (char*)s : start);
    return minus ? -total : total;
}

#ifndef _REENT_ONLY

unsigned long _DEFUN(strtoul, (s, ptr, base), _CONST char* s _AND char** ptr _AND int base)
{
