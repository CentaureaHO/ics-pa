...
static _CONST int DAYS_IN_MONTH[12] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};

#define _DAYS_IN_MONTH(x) ((x == 1) ? days_in_feb : DAYS_IN_MONTH[x])

static _CONST int _DAYS_BEFORE_MONTH[12] = {0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334};

#define _DAYS_IN_YEAR(year) (((year) % 4) ? 365 : 366)

static void validate_structure(tim_p) struct tm* tim_p;
{
    div_t res;
    int   days_in_feb = 28;
    ...
    if (tim_p->tm_sec < 0 || tim_p->tm_sec > 59)
    {
        ...
        if ((tim_p->tm_sec = res.rem) < 0) tim_p->tm_sec += 60;
    }
    ...
    if (tim_p->tm_min < 0 || tim_p->tm_min > 59)
    {
        ...
        if ((tim_p->tm_min = res.rem) < 0) tim_p->tm_min += 60;
    }
    ...
    if (tim_p->tm_hour < 0 || tim_p->tm_hour > 23)
    {
        ...
        if ((tim_p->tm_hour = res.rem) < 0) tim_p->tm_hour += 24;
    }
    ...
    if (tim_p->tm_mon > 11)
    {
        ...
        if ((tim_p->tm_mon = res.rem) < 0) tim_p->tm_mon += 12;
    }
