...
char* _DEFUN(_llicvt, (buffer, value, type), char* buffer _AND long long value _AND char type)
{
...
char* _DEFUN(_licvt, (buffer, value, type), char* buffer _AND long value _AND char type)
{
    int base, i;
    ...
    switch (type)
    {
        ...
        case 'X':
            ...
            while (i >= 0)
            {
                ...
                if ((value = ((unsigned long)value) / base) == 0) break;
            }
            break;
...
char* _DEFUN(_icvt, (buffer, value, type), char* buffer _AND int value _AND char type)
{
    int base, i;
    ...
    switch (type)
    {
        ...
        case 'X':
            ...
            while (i >= 0)
            {
                ...
                if ((value = ((unsigned)value) / base) == 0) break;
            }
            break;
...
char* _DEFUN(_sicvt, (buffer, value, type), char* buffer _AND short value _AND char type)
{
    if (sizeof(short) == sizeof(int)) { return (_icvt(buffer, value, type)); }
    else
    {
        int base, i;
        i           = CVT_BUF_SIZE - 1;
