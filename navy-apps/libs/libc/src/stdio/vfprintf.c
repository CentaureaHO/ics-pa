...
static int out_string(s, n_chars, flags, n_zero, n_pad, prefix, stream)
char*      s;
...
FILE*      stream;
{
    ...
    if (n_pad && !(flags & FMINUS))
    {
        if (prefix && (flags & FZERO))
            if (fputs(prefix, stream) == -1) return -1;

        for (i = 0; i < n_pad; i++)
        {
            if (flags & FZERO)
            {
                if (fputc('0', stream) == EOF) return -1;
            }
        ...
        if (prefix && !(flags & FZERO))
            if (fputs(prefix, stream) != 0) return -1;
    }
    else if (prefix)
        if (fputs(prefix, stream) != 0) return -1;
    ...
    for (i = 0; i < n_zero; i++)
        if (fputc('0', stream) == EOF) return -1;
    for (i = 0; i < n_chars; i++)
        if (fputc(*s++, stream) == EOF) return -1;
...
int _DEFUN(VFPRINTF, (stream, format, args), FILE* stream _AND _CONST char* format _AND va_list args)
{
    return _VFPRINTF_R(stream->_data, stream, format, args);
}

int _DEFUN(_VFPRINTF_R, (data, stream, format, args),
    struct _reent* data _AND FILE* stream _AND _CONST char* format _AND va_list args)
{
    ...
    for (;;)
    {
        if (!flags)
        {
            while (*format && *format != '%')
            {
                if (fputc(*format++, stream) == EOF) return -1;
                count++;
            }
        ...
        switch (*format++)
        {
            case '-': flags |= FMINUS; break;
            case '+': flags |= FPLUS; break;
            case ' ': flags |= FSPACE; break;
            case '#': flags |= FHASH; break;
            ...
            case '9':
                if (flags & FDOT)
                    precision = precision * 10 + *(format - 1) - '0';
                else
                    field_width = field_width * 10 + *(format - 1) - '0';
                break;
            case '*':
                if (flags & FDOT)
            ...
            case 'h': flags |= FSHORT; break;
            case 'l': flags |= FLONG; break;
            case 'L': flags |= FDLONG; break;
            ...
            case 'X':
                ...
                if (precision != UNSET) flags &= ~FZERO;

                if (precision == UNSET) precision = 1;

                f_type = *(format - 1);

                if (precision || sign)
                {
                    if (flags & FSHORT) fld = _sicvt(buffer, (short)i, f_type);
#ifdef __GNUC__
                ...
                n_zero = 0;
                if (precision > n_chars) n_zero = precision - n_chars;

                n_pad = 0;
                if (field_width > n_chars + n_zero) n_pad = field_width - n_chars - n_zero;
                ...
                if (n_pad && prefix) n_pad -= strlen(prefix);

                if ((n_chars = out_string(fld, n_chars, flags, n_zero, n_pad, prefix, stream)) == -1)
                    return -1;
                else
                    count += n_chars;
            ...
            case 'G':
            {
                ...
                if (precision == UNSET || precision < 0) precision = 6;

                f_type = *(format - 1);

                fld = _dcvt(data, buffer, d, precision, field_width, f_type, flags & FHASH);

                n_chars = (fld ? strlen(fld) : 0);

                n_pad = 0;
                if (field_width > n_chars) n_pad = field_width - n_chars;
                ...
                if ((n_chars = out_string(fld, n_chars, flags, 0, n_pad, prefix, stream)) == -1)
                    return -1;
                else
                    count += n_chars;
            ...
            case 's':
                ...
                n_chars = strlen(s);
                if (precision != UNSET && precision < n_chars) n_chars = precision;

                n_pad = 0;
                if (field_width > n_chars) n_pad = field_width - n_chars;

                if ((n_chars = out_string(s, n_chars, flags, 0, n_pad, "", stream)) == -1)
                    return -1;
                else
                    count += n_chars;
            ...
            case 'c':
                ...
                n_pad = 0;
                if (field_width > 1) n_pad = field_width - 1;

                if ((n_chars = out_string(&c, 1, flags, 0, n_pad, "", stream)) == -1)
                    return -1;
                else
                    count += n_chars;
            ...
            case 'p': ip = va_arg(args, int*);
                ...
                if ((n_chars = out_string(fld, n_chars, flags, n_zero, 0, prefix, stream)) == -1)
                    return -1;
                else
                    count += n_chars;
            ...
            case '%':
                ...
                if ((n_chars = out_string("%", 1, flags, 0, n_pad, "", stream)) == -1)
                    return -1;
                else
                    count += n_chars;
