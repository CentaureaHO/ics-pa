    ...
    switch (sig)
    {
        ...
        case SIGTERM:
            ...
            if (ptr->sig_func == NULL)
            {
                ...
                if (ptr->sig_func == NULL) { return SIG_ERR; }
            }
...
_sig_func_ptr _DEFUN(signal, (sig, func), int sig _AND _sig_func_ptr func) { return _signal_r(_REENT, sig, func); }
