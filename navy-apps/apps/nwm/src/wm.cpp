...
void WindowManager::render()
{
    ...
    if (focus)
    {
        ...
        {
            if (win && win != focus) { draw_window(win); }
        }
        draw_window(focus);
        if (display_switcher)
        {
    ...
    if (display_appfinder) { draw_window(appfinder); }
...
{
    if (x >= 0 && y >= 0 && x < w && y < h) { changed[tw * (y >> tile_shift) + (x >> tile_shift)] = true; }
}
