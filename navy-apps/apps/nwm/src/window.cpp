...
void Window::draw_px(int x, int y, uint32_t color) { draw_raw_px(x + dx, y + dy, color); }
...
void Window::draw_ch(Font* font, int x, int y, char ch, uint32_t color)
{
    ...
    for (int j = 0; j < font->h; j++)
        for (int i = 0; i < font->w; i++)
            if ((bm && ((bm[j] >> i) & 1))) { draw_px(x + i, y + j, color); }
}
...
void Window::draw_raw_ch(Font* font, int x, int y, char ch, uint32_t color)
{
    ...
    for (int j = 0; j < font->h; j++)
        for (int i = 0; i < font->w; i++)
            if ((bm && ((bm[j] >> i) & 1))) { draw_raw_px(x + i, y + j, color); }
}

void Window::draw()
{
    for (int i = 0; i < w; i++)
        for (int j = 0; j < h; j++) { wm->mark_dirty(this->x + i, this->y + j); }
}
...
void Window::center() { move((wm->w - w) / 2, (wm->h - h) / 2); }
...
void Window::resize(int width, int height)
{
    ...
    for (int i = 0; i < w; i++)
        for (int j = 0; j < h; j++) draw_raw_px(i, j, border_col);
...
void Window::update()
{
    if (read_fd != -1)
    {
        ...
        do {
            ...
            {
                if (esc_state.feed(buf[i])) { draw_px(esc_state.x, esc_state.y, esc_state.px); }
            }
        } while (1);
    }
}
