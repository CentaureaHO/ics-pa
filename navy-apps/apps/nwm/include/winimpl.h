...
class WindowSwitcher : public Window
{
    ...
    void project(Window* win, int basex, int basey, int w, int h)
    {
        ...
        for (int x = 0; x < w1; x++)
            for (int y = 0; y < h1; y++)
            {
                ...
                if (col & 0xff000000) { col = border_col; }
  ...
  public:
    WindowSwitcher(WindowManager* wm, int width, int height) : Window(wm, nullptr, nullptr, nullptr)
    {
                ...
                if (win == wm->focus)
                {
                    for (int i = 0; i < preview_px + 4; i++)
                        for (int j = 0; j < preview_px + 4; j++)
                            ...
                            else { draw_px(x + i - 2, preview_pad + j - 2, 0x81bef7); }
                }
                if (win)
                {
  ...
  public:
    ...
    AppFinder(WindowManager* wm) : Window(wm, nullptr, nullptr, nullptr)
    {
        ...
        for (int x = 0; x < w; x++)
        {
            for (int y = 0; y < h; y++)
            {
                if (y >= cutline) { draw_px(x, y, border_col); }
                else { draw_px(x, y, 0xff000000); }
            }
        }
        sync();
    }
    ...
    void draw_item(int idx, int x, int y)
    {
        ...
        if (idx == cur)
        {
            ...
            {
                for (int j = 0; j < item_size; j++) { draw_px(x + i, y + j, 0xc0c0f0); }
            }
        }
        else
        {
            for (int i = 0; i < item_size; i++)
                ...
                {
                    if (y + j >= cutline) { draw_px(x + i, y + j, border_col); }
                    else { draw_px(x + i, y + j, 0xff000000); }
                }
        }
        ...
        for (int i = 0; i < icon_size; i++)
            for (int j = 0; j < icon_size; j++) { draw_px(x + icon_dx + i, y + icon_dy + j, 0xf0f0f0); }
