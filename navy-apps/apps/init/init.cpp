...
int main(int argc, char* argv[], char* envp[])
{
    ...
    if (!freopen("/dev/tty", "r", stdin) || !freopen("/dev/tty", "w", stdout) || !freopen("/dev/tty", "w", stderr))
    {
    ...
    while (1)
    {
        ...
        do {
            char cmd[32], data[32];
            if (fscanf(evtdev, "%s %s", cmd, data) != 2) { continue; }
            if (strcmp(cmd, "kd") == 0) { sscanf(data, "%d", &i); }
        } while (i == 0);
        ...
        else { fprintf(stderr, "Choose a number between %d and %d\n\n", 1, nitems); }
    }
    return -1;
}
...
static void open_display()
{
    ...
    if (!fbdev) { fprintf(stderr, "[IGN] Cannot open display.\n"); }
}
