...
static int addk(FuncState* fs, TValue* key, TValue* v)
{
    ...
    { /* is there an index there? */
        ...
        if (k < fs->nk && ttype(&f->k[k]) == ttype(v) && luaV_rawequalobj(&f->k[k], v)) return k; /* reuse index */
    }
...
{
    if (e->k == VCALL) { /* expression is an open function call? */ SETARG_C(getinstruction(fs, e), nresults + 1); }
    else if (e->k == VVARARG)
    {
...
void luaK_posfix(FuncState* fs, BinOpr op, expdesc* e1, expdesc* e2, int line)
{
    switch (op)
    {
        ...
        case OPR_SHR:
        {
            if (!constfolding(fs, op + LUA_OPADD, e1, e2)) codebinexpval(fs, cast(OpCode, op + OP_ADD), e1, e2, line);
            break;
        }
        case OPR_EQ:
        case OPR_LT:
