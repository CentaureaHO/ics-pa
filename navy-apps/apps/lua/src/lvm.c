...
void luaV_finishset(lua_State* L, const TValue* t, TValue* key, StkId val, const TValue* slot)
{
    ...
    for (loop = 0; loop < MAXTAGLOOP; loop++)
    {
        ...
        { /* not a table; check metamethod */
            if (ttisnil(tm = luaT_gettmbyobj(L, t, TM_NEWINDEX))) luaG_typeerror(L, t, "index");
        }
        /* try the metamethod */
        if (ttisfunction(tm))
        {
    ...
    do {
        ...
        else
        {
            ...
            for (n = 1; n < total && tostring(L, top - n - 1); n++)
            {
                ...
                if (l >= (MAX_SIZE / sizeof(char)) - tl) luaG_runerror(L, "string length overflow");
                tl += l;
            }
...
#define settableProtected(L, t, k, v)                                                             \
    {                                                                                             \
        const TValue* slot;                                                                       \
        if (!luaV_fastset(L, t, k, slot, luaH_get, v)) Protect(luaV_finishset(L, t, k, v, slot)); \
    }
    ...
    for (;;)
    {
        ...
        vmdispatch(GET_OPCODE(i))
        {
            ...
            vmcase(OP_SELF)
            {
                ...
                setobjs2s(L, ra + 1, rb);
                if (luaV_fastget(L, rb, key, aux, luaH_getstr)) { setobj2s(L, ra, aux); }
                else
            ...
            vmcase(OP_ADD)
            {
                ...
                else if (tonumber(rb, &nb) && tonumber(rc, &nc)) { setfltvalue(ra, luai_numadd(L, nb, nc)); }
            ...
            vmcase(OP_SUB)
            {
                ...
                else if (tonumber(rb, &nb) && tonumber(rc, &nc)) { setfltvalue(ra, luai_numsub(L, nb, nc)); }
            ...
            vmcase(OP_MUL)
            {
                ...
                else if (tonumber(rb, &nb) && tonumber(rc, &nc)) { setfltvalue(ra, luai_nummul(L, nb, nc)); }
            ...
            { /* float division (always with floats) */
                ...
                lua_Number nb;
                lua_Number nc;
                if (tonumber(rb, &nb) && tonumber(rc, &nc)) { setfltvalue(ra, luai_numdiv(L, nb, nc)); }
            ...
            vmcase(OP_BAND)
            {
                ...
                lua_Integer ib;
                lua_Integer ic;
                if (tointeger(rb, &ib) && tointeger(rc, &ic)) { setivalue(ra, intop(&, ib, ic)); }
            ...
            vmcase(OP_SHL)
            {
                ...
                lua_Integer ib;
                lua_Integer ic;
                if (tointeger(rb, &ib) && tointeger(rc, &ic)) { setivalue(ra, luaV_shiftl(ib, ic)); }
            ...
            { /* floor division */
                ...
                else if (tonumber(rb, &nb) && tonumber(rc, &nc)) { setfltvalue(ra, luai_numidiv(L, nb, nc)); }
            ...
            vmcase(OP_POW)
            {
                ...
                lua_Number nb;
                lua_Number nc;
                if (tonumber(rb, &nb) && tonumber(rc, &nc)) { setfltvalue(ra, luai_numpow(L, nb, nc)); }
            ...
            vmcase(OP_UNM)
            {
                ...
                else if (tonumber(rb, &nb)) { setfltvalue(ra, luai_numunm(L, nb)); }
            ...
            vmcase(OP_TAILCALL)
            {
                ...
                if (luaD_precall(L, ra, LUA_MULTRET)) { /* C function? */ Protect((void)0); /* update 'base' */ }
                else
                {
            ...
            vmcase(OP_FORPREP)
            {
                ...
                { /* try making all values floats */
                    ...
                    lua_Number nlimit;
                    lua_Number nstep;
                    if (!tonumber(plimit, &nlimit)) luaG_runerror(L, "'for' limit must be a number");
                    setfltvalue(plimit, nlimit);
                    if (!tonumber(pstep, &nstep)) luaG_runerror(L, "'for' step must be a number");
                    setfltvalue(pstep, nstep);
                    if (!tonumber(init, &ninit)) luaG_runerror(L, "'for' initial value must be a number");
