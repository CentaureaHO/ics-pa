...
static lua_Number lua_strx2number(const char* s, char** endptr)
{
    ...
    *endptr = cast(char*, s);   /* valid up to here */
    ...
    {                                              /* exponent part? */
        ...
        s++;                                       /* skip 'p' */
        neg1 = isneg(&s);                          /* signal */
        if (!lisdigit(cast_uchar(*s))) return 0.0; /* invalid; must have at least one digit */
...
static const char* l_str2d(const char* s, lua_Number* result)
{
    ...
    { /* failed? may be a different locale */
        ...
        if (strlen(s) > L_MAXLENNUM || pdot == NULL) return NULL; /* string too long or no dot; fail */
...
size_t luaO_str2num(const char* s, TValue* o)
{
    ...
    lua_Number  n;
    const char* e;
    if ((e = l_str2int(s, &i)) != NULL) { /* try as an integer */ setivalue(o, i); }
    else if ((e = l_str2d(s, &n)) != NULL) { /* else try as a float */ setfltvalue(o, n); }
    else
...
void luaO_chunkid(char* out, const char* source, size_t bufflen)
{
    ...
    {                                          /* string; format as [string "source"] */
        ...
        if (l < bufflen && nl == NULL) { /* small one-line source? */ addstr(out, source, l); /* keep it */ }
        else
        {
