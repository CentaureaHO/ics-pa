...
static int str_dump(lua_State* L)
{
    ...
    lua_settop(L, 1);
    luaL_buffinit(L, &b);
    if (lua_dump(L, writer, &b, strip) != 0) return luaL_error(L, "unable to dump given function");
    luaL_pushresult(&b);
    return 1;
}
...
static void add_s(MatchState* ms, luaL_Buffer* b, const char* s, const char* e)
{
    ...
    for (i = 0; i < l; i++)
    {
        ...
        else
        {
            ...
            {
                if (news[i] != L_ESC) luaL_error(L, "invalid use of '%c' in replacement string", L_ESC);
                luaL_addchar(b, news[i]);
            }
            else if (news[i] == '0')
...
static const char* scanformat(lua_State* L, const char* strfrmt, char* form)
{
    ...
    if ((size_t)(p - strfrmt) >= sizeof(FLAGS) / sizeof(char)) luaL_error(L, "invalid format (repeated flags)");
...
static int getnumlimit(Header* h, const char** fmt, int df)
{
    ...
    if (sz > MAXINTSIZE || sz <= 0) luaL_error(h->L, "integral size (%d) out of limits [1,%d]", sz, MAXINTSIZE);
    return sz;
}
...
static int str_unpack(lua_State* L)
{
    ...
    while (*fmt != '\0')
    {
        ...
        if ((size_t)ntoalign + size > ~pos || pos + ntoalign + size > ld) luaL_argerror(L, 2, "data string too short");
