...
void Terminal::esc_move(int* args) { cursor = {.x = args[1] - 1, .y = args[0] - 1}; }

void Terminal::esc_movefirst(int* args) { cursor = {.x = 0, .y = 0}; }

void Terminal::esc_moveup(int* args) { cursor.y -= args[0]; }

void Terminal::esc_movedown(int* args) { cursor.y += args[0]; }

void Terminal::esc_moveleft(int* args) { cursor.x -= args[0]; }

void Terminal::esc_moveright(int* args) { cursor.x += args[0]; }

void Terminal::esc_save(int* args) { saved = cursor; }

void Terminal::esc_restore(int* args) { cursor = saved; }

void Terminal::esc_clear(int* args)
{
    for (int i = 0; i < w; i++)
        for (int j = 0; j < h; j++) { putch(i, j, EMPTY); }
    cursor = {.x = 0, .y = 0};
}
...
{
    for (int i = cursor.x; i < w; i++) { putch(i, cursor.y, EMPTY); }
}

void Terminal::esc_rawmode(int* args) { mode = Mode::raw; }

void Terminal::esc_cookmode(int* args) { mode = Mode::cook; }
...
Terminal::Terminal(int width, int height)
{
    ...
    for (int x = 0; x < w; x++)
    {
        for (int y = 0; y < h; y++) { putch(x, y, EMPTY); }
    }
}
...
void Terminal::scroll_up()
{
    ...
    for (int i = 0; i < w; i++) { putch(i, h - 1, EMPTY); }
    for (int i = 0; i < w * h; i++) { dirty[i] = true; }
}
...
size_t Terminal::write_escape(const char* str, size_t count)
{
    for (auto& p : esc_seqs)
    {
        ...
        for (const char *cur = p.pattern, *s = str;; cur++)
        {
            ...
            {
                int data = 0;
                for (; *s >= '0' && *s <= '9' && s - str < count; s++, len++) { data = data * 10 + *s - '0'; }
                args[narg++] = data;
            }
        }
...
void Terminal::write(const char* str, size_t count)
{
    ...
    {
        char ch = str[i];
        if (ch == '\033') { i += write_escape(&str[i], count - i); }
        else
        {
            switch (ch)
            {
...
char Terminal::getch(int x, int y) { return buf[x + y * w]; }
...
uint32_t Terminal::foreground(int x, int y) { return colors[color[x + y * w] >> 4]; }

uint32_t Terminal::background(int x, int y) { return colors[color[x + y * w] & 0xf]; }

bool Terminal::is_dirty(int x, int y) { return dirty[x + y * w]; }
