...
--*/
{
    ...
    {
        if (g_Battle.rgEnemy[i].wObjectID != 0 && g_Battle.rgEnemy[i].e.wHealth > 0) { return i; }
    }
...
static SHORT PAL_CalcMagicDamage(WORD wMagicStrength, WORD wDefense,
    const WORD rgwElementalResistance[NUM_MAGIC_ELEMENTAL], WORD wPoisonResistance, WORD wMagicID)
/*++
   Purpose:
...
--*/
{
    ...
    if (gpGlobals->g.lprgMagic[wMagicID].wElemental != 0)
    {
        ...
        if (wElem > NUM_MAGIC_ELEMENTAL) { sDamage *= 10 - wPoisonResistance; }
        else if (wElem == 0) { sDamage *= 5; }
        else { sDamage *= 10 - rgwElementalResistance[wElem - 1]; }
...
--*/
{
    ...
    if (wAttackResistance != 0) { sDamage /= wAttackResistance; }
...
#ifdef PAL_CLASSIC
    if (wDexterity > 999) { wDexterity = 999; }
#endif
...
--*/
{
    ...
    g_Battle.flTimeChargingUnit = F_div_int(g_Battle.flTimeChargingUnit, PAL_GetPlayerDexterity(0));
...
--*/
{
    ...
    //
    if (!gpGlobals->fAutoBattle) { wDexterity /= 4; }
...
--*/
{
    ...
    for (i = 0; i < wDuration; i++)
    {
            ...
            for (j = 0; j <= g_Battle.wMaxEnemyIndex; j++)
            {
                ...
                if (--g_Battle.rgEnemy[j].e.wIdleAnimSpeed == 0)
                {
                    ...
                    g_Battle.rgEnemy[j].e.wIdleAnimSpeed =
                        gpGlobals->g.lprgEnemy[gpGlobals->g.rgObject[g_Battle.rgEnemy[j].wObjectID].enemy.wEnemyID]
                            .wIdleAnimSpeed;
                }
        ...
        if (wObjectID != 0)
        {
            ...
            else { PAL_DrawText(PAL_GetWord(wObjectID), PAL_XY(210, 50), 15, TRUE, FALSE); }
        }
...
--*/
{
    ...
    for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++)
    {
        if (g_Battle.rgEnemy[i].wObjectID == 0) { continue; }
...
--*/
{
    ...
    for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++)
    {
        ...
        if (g_Battle.rgEnemy[i].wPrevHP != g_Battle.rgEnemy[i].e.wHealth)
        {
            ...
            if (sDamage < 0) { PAL_BattleUIShowNum((WORD)(-sDamage), PAL_XY(x, y), kNumColorBlue); }
            else { PAL_BattleUIShowNum((WORD)(sDamage), PAL_XY(x, y), kNumColorYellow); }
    ...
    for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
    {
        ...
        if (g_Battle.rgPlayer[i].wPrevMP != gpGlobals->g.PlayerRoles.rgwMP[wPlayerRole])
        {
            ...
            //
            if (sDamage > 0) { PAL_BattleUIShowNum((WORD)(sDamage), PAL_XY(x, y), kNumColorCyan); }
    ...
    if (fCheckPlayers && !gpGlobals->fAutoBattle)
    {
        for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
        {
                ...
                gpGlobals->g.PlayerRoles.rgwHP[w] == 0)
            {
                ...
                {
                    if (gpGlobals->rgParty[j].wPlayerRole == w) { break; }
                }

                if (gpGlobals->g.PlayerRoles.rgwHP[w] > 0 && gpGlobals->rgPlayerStatus[w][kStatusSleep] == 0 &&
                    gpGlobals->rgPlayerStatus[w][kStatusParalyzed] == 0 &&
                    gpGlobals->rgPlayerStatus[w][kStatusConfused] == 0 && j <= gpGlobals->wMaxPartyMemberIndex)
                {
        ...
        for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
        {
            ...
            if (gpGlobals->rgPlayerStatus[w][kStatusSleep] != 0 || gpGlobals->rgPlayerStatus[w][kStatusConfused] != 0)
            {
                continue;
            }
            ...
            if (gpGlobals->g.PlayerRoles.rgwHP[w] < g_Battle.rgPlayer[i].wPrevHP)
            {
                    ...
                    g_Battle.rgPlayer[i].wPrevHP >= gpGlobals->g.PlayerRoles.rgwMaxHP[w] / 5)
                {
                    ...
                    for (j = 0; j <= gpGlobals->wMaxPartyMemberIndex; j++)
                    {
                        if (gpGlobals->rgParty[j].wPlayerRole == wCover) { break; }
                    }
...
--*/
{
    ...
    for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
    {
        ...
        {
            if (gpGlobals->rgPlayerStatus[wPlayerRole][kStatusSleep] != 0 || PAL_IsPlayerDying(wPlayerRole))
            {
                g_Battle.rgPlayer[i].wCurrentFrame = 1;
            }
#ifndef PAL_CLASSIC
            else if (g_Battle.rgPlayer[i].state == kFighterAct &&
                     g_Battle.rgPlayer[i].action.ActionType == kBattleActionMagic && !g_Battle.fEnemyCleared)
            {
                //
            ...
            else { g_Battle.rgPlayer[i].wCurrentFrame = 0; }
        }
    }
    ...
    for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++)
    {
        ...
        if (--g_Battle.rgEnemy[i].e.wIdleAnimSpeed == 0)
        {
            ...
            g_Battle.rgEnemy[i].e.wIdleAnimSpeed =
                gpGlobals->g.lprgEnemy[gpGlobals->g.rgObject[g_Battle.rgEnemy[i].wObjectID].enemy.wEnemyID]
                    .wIdleAnimSpeed;
        }
...
--*/
{
    ...
    if (!g_Battle.fEnemyCleared) { PAL_BattleUpdateFighters(); }
    ...
    else
    {
        ...
        for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
        {
            ...
            else if (gpGlobals->rgPlayerStatus[wPlayerRole][kStatusPuppet] != 0) { fEnded = FALSE; }
        }
    ...
    {
        if (PAL_GetTimeChargingSpeed(9999) > 0) { g_Battle.iHidingTime--; }
    ...
    for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++)
    {
        ...
        {
            g_Battle.rgEnemy[i].wScriptOnTurnStart = PAL_RunTriggerScript(g_Battle.rgEnemy[i].wScriptOnTurnStart, i);
        ...
        switch (g_Battle.rgEnemy[i].state)
        {
            case kFighterWait:
                ...
                if (flMax != 0)
                {
                    ...
                    {
                        if (g_Battle.iHidingTime == 0) { g_Battle.rgEnemy[i].state = kFighterCom; }
                        else { g_Battle.rgEnemy[i].flTimeMeter = 0; }
                    }
                }
                break;

            case kFighterCom:
                g_Battle.rgEnemy[i].wScriptOnReady = PAL_RunTriggerScript(g_Battle.rgEnemy[i].wScriptOnReady, i);
            ...
            case kFighterAct:
                if (!fMoved && (PAL_GetTimeChargingSpeed(9999) > 0 || g_Battle.rgEnemy[i].fDualMove) && !fOnlyPuppet)
                {
                    ...
                    g_Battle.rgEnemy[i].fDualMove = (!g_Battle.rgEnemy[i].fFirstMoveDone &&
        ...
        switch (g_Battle.rgPlayer[i].state)
        {
            case kFighterWait:
                ...
                g_Battle.rgPlayer[i].flTimeMeter +=
                    F_mul_F(PAL_GetTimeChargingSpeed(wDexterity), g_Battle.rgPlayer[i].flTimeSpeedModifier);
                break;
            ...
            case kFighterAct:
                ...
                if (g_Battle.rgPlayer[i].action.flRemainingTime <= 0 && g_Battle.rgPlayer[i].sTurnOrder == -1)
                {
                    sMax = -1;

                    for (j = 0; j <= gpGlobals->wMaxPartyMemberIndex; j++)
                    {
                        if (g_Battle.rgPlayer[j].sTurnOrder > sMax) { sMax = g_Battle.rgPlayer[j].sTurnOrder; }
                    }
            ...
            if (i > gpGlobals->wMaxPartyMemberIndex)
            {
                ...
                for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++)
                {
                    ...
                    g_Battle.ActionQueue[j].wDexterity =
                        F2int(F_mul_int(RandomFloat(f2F(0.9), f2F(1.1)), g_Battle.ActionQueue[j].wDexterity));
                ...
                for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
                {
                    ...
                    else
                    {
                        ...
                        if (PAL_IsPlayerDying(wPlayerRole)) { wDexterity /= 2; }

                        wDexterity = F2int(F_mul_int(RandomFloat(f2F(0.9), f2F(1.1)), wDexterity));
    ...
    else
    {
        ...
        {
            for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) { g_Battle.rgPlayer[i].fDefending = FALSE; }
            ...
            for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
            {
                ...
                {
                    if (gpGlobals->rgPlayerStatus[wPlayerRole][j] > 0) { gpGlobals->rgPlayerStatus[wPlayerRole][j]--; }
                }
            }
            ...
            for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++)
            {
                ...
                for (j = 0; j < kStatusAll; j++)
                {
                    if (g_Battle.rgEnemy[i].rgwStatus[j] > 0) { g_Battle.rgEnemy[i].rgwStatus[j]--; }
                }
            }

            PAL_BattlePostActionCheck(FALSE);
            if (PAL_BattleDisplayStatChange()) { PAL_BattleDelay(8, 0, TRUE); }
            ...
            //
            for (i = 0; i < MAX_INVENTORY; i++) { gpGlobals->rgInventory[i].nAmountInUse = 0; }
    ...
    {
        if (g_InputState.dwKeyPress & kKeyRepeat) { g_Battle.fRepeat = TRUE; }
        else if (g_InputState.dwKeyPress & kKeyForce) { g_Battle.fForce = TRUE; }
    }

    if (g_Battle.fRepeat) { g_InputState.dwKeyPress = kKeyRepeat; }
    else if (g_Battle.fForce) { g_InputState.dwKeyPress = kKeyForce; }
    else if (g_Battle.fFlee) { g_InputState.dwKeyPress = kKeyFlee; }
...
--*/
{
    ...
    if (!fRepeat)
    {
        g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action.ActionType = g_Battle.UI.wActionType;
        g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action.sTarget    = (SHORT)g_Battle.UI.wSelectedIndex;
    ...
    switch (g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action.ActionType)
    {
        ...
        case kBattleActionUseItem:
            if ((gpGlobals->g.rgObject[g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action.wActionID].item.wFlags &
...
#else
    if (g_Battle.UI.wActionType == kBattleActionFlee) { g_Battle.fFlee = TRUE; }
#endif
    ...
    if (g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action.flRemainingTime <= 0)
    {
        ...
        {
            if (g_Battle.rgPlayer[w].sTurnOrder > sMax) { sMax = g_Battle.rgPlayer[w].sTurnOrder; }
        }

        g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].sTurnOrder = sMax + 1;
    }
    else { g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].sTurnOrder = -1; }
#endif
}
...
--*/
{
    ...
    if (sTarget != -1)
    {
        ...
        enemy_h = PAL_RLEGetHeight(
            PAL_SpriteGetFrame(g_Battle.rgEnemy[sTarget].lpSprite, g_Battle.rgEnemy[sTarget].wCurrentFrame));

        if (sTarget >= 3) { dist = (sTarget - wPlayerIndex) * 8; }
    }
    else
    {
        enemy_x = 150;
    ...
    if (gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole] > 0)
    {
        if (!fCritical) { SOUND_Play(gpGlobals->g.PlayerRoles.rgwAttackSound[wPlayerRole]); }
        else { SOUND_Play(gpGlobals->g.PlayerRoles.rgwCriticalSound[wPlayerRole]); }
    }
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        if (i == 0)
        {
            if (sTarget == -1)
            {
                for (j = 0; j <= g_Battle.wMaxEnemyIndex; j++) { g_Battle.rgEnemy[j].iColorShift = 6; }
            }
            else { g_Battle.rgEnemy[sTarget].iColorShift = 6; }
        ...
        if (i == 1)
        {
            g_Battle.rgPlayer[wPlayerIndex].pos =
                PAL_XY(PAL_X(g_Battle.rgPlayer[wPlayerIndex].pos) + 2, PAL_Y(g_Battle.rgPlayer[wPlayerIndex].pos) + 1);
        }
    }

    dist = 8;

    for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++) { g_Battle.rgEnemy[i].iColorShift = 0; }
...
--*/
{
    ...
    g_Battle.rgPlayer[wPlayerIndex].pos =
        PAL_XY(PAL_X(g_Battle.rgPlayer[wPlayerIndex].pos) - 15, PAL_Y(g_Battle.rgPlayer[wPlayerIndex].pos) - 7);
    ...
    for (i = 0; i <= 6; i++)
    {
        if (sTarget == -1)
        {
            for (j = 0; j <= gpGlobals->wMaxPartyMemberIndex; j++) { g_Battle.rgPlayer[j].iColorShift = i; }
        }
        else { g_Battle.rgPlayer[sTarget].iColorShift = i; }
...
--*/
{
    ...
    for (i = 0; i < 4; i++)
    {
        g_Battle.rgPlayer[wPlayerIndex].pos = PAL_XY(PAL_X(g_Battle.rgPlayer[wPlayerIndex].pos) - (4 - i),
...
--*/
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        // Set the time of the next frame.
        //
        dwTime = SDL_GetTicks() + ((SHORT)(gpGlobals->g.lprgMagic[iMagicNum].wSpeed) + 5) * 10;
            ...
            if (sTarget > 0 && g_Battle.iHidingTime == 0)
            {
                if (gpGlobals->rgPlayerStatus[gpGlobals->rgParty[sTarget - 1].wPlayerRole][kStatusConfused] == 0)
                {
                    LPCBITMAPRLE p = PAL_SpriteGetFrame(
                        g_Battle.rgPlayer[sTarget - 1].lpSprite, g_Battle.rgPlayer[sTarget - 1].wCurrentFrame);
...
--*/
{
...
#ifdef PAL_WIN95
    if (wPlayerIndex != (WORD)-1) { g_Battle.rgPlayer[wPlayerIndex].wCurrentFrame = 6; }
#endif
    ...
    for (i = 0; i < l; i++)
    {
        ...
        {
            if (g_Battle.rgEnemy[k].wObjectID == 0) { continue; }
        ...
        if (gpGlobals->g.lprgMagic[iMagicNum].wType == kMagicTypeNormal)
        {
            ...
            if (i == l - 1 && gpGlobals->wScreenWave < 9 && gpGlobals->g.lprgMagic[iMagicNum].wKeepEffect == 0xFFFF)
            {
                PAL_RLEBlitToSurface(
                    b, g_Battle.lpBackground, PAL_XY(x - PAL_RLEGetWidth(b) / 2, y - PAL_RLEGetHeight(b)));
            }
        }
    ...
    for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++) { g_Battle.rgEnemy[i].pos = g_Battle.rgEnemy[i].posOriginal; }
}
...
--*/
{
    ...
    {
        if (gpGlobals->g.rgObject[wEffectMagicID].magic.wMagicNumber == gpGlobals->g.lprgMagic[wMagicNum].wEffect)
        {
            break;
        }
    }
    ...
    while (g_Battle.iSummonFrame < PAL_SpriteGetNumFrames(g_Battle.lpSummonSprite) - 1)
    {
        ...
        // Set the time of the next frame.
        //
        dwTime = SDL_GetTicks() + ((SHORT)(gpGlobals->g.lprgMagic[wMagicNum].wSpeed) + 5) * 10;
...
--*/
{
    ...
    for (i = 0; i < MAX_ENEMIES_IN_TEAM; i++) { rgEnemyPosBak[i] = g_Battle.rgEnemy[i].pos; }

    for (i = 0; i < 3; i++)
    {
        for (j = 0; j <= g_Battle.wMaxEnemyIndex; j++)
        {
            if (g_Battle.rgEnemy[j].e.wHealth == g_Battle.rgEnemy[j].wPrevHP) { continue; }
    ...
    for (i = 0; i < MAX_ENEMIES_IN_TEAM; i++) { g_Battle.rgEnemy[i].pos = rgEnemyPosBak[i]; }
...
--*/
{
    ...
    switch (g_Battle.rgPlayer[wPlayerIndex].action.ActionType)
    {
        ...
        case kBattleActionMagic:
            ...
            if (i >= MAX_PLAYER_MAGICS) { fValid = FALSE; }
            ...
            {
                if (!fValid) { g_Battle.rgPlayer[wPlayerIndex].action.ActionType = kBattleActionAttack; }
            ...
            {
                if (!fValid) { g_Battle.rgPlayer[wPlayerIndex].action.ActionType = kBattleActionDefend; }
                else if (gpGlobals->g.rgObject[wObjectID].magic.wFlags & kMagicFlagApplyToAll)
                {
                    g_Battle.rgPlayer[wPlayerIndex].action.sTarget = -1;
                }
        ...
        case kBattleActionCoopMagic:
            ...
            for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
            {
                ...
                if (PAL_IsPlayerDying(w) || gpGlobals->rgPlayerStatus[w][kStatusSilence] > 0 ||
                    gpGlobals->rgPlayerStatus[w][kStatusSleep] > 0 ||
                    gpGlobals->rgPlayerStatus[w][kStatusConfused] > 0 || g_Battle.rgPlayer[i].flTimeMeter < 100 ||
        ...
        case kBattleActionAttackMate:
            ...
            else
            {
                for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
                {
                    if (i != wPlayerIndex && gpGlobals->g.PlayerRoles.rgwHP[gpGlobals->rgParty[i].wPlayerRole] != 0)
                    {
                        break;
                    }
                }
    ...
    if (g_Battle.rgPlayer[wPlayerIndex].action.ActionType == kBattleActionAttack)
    {
        ...
        else if (PAL_PlayerCanAttackAll(wPlayerRole)) { g_Battle.rgPlayer[wPlayerIndex].action.sTarget = -1; }
    }
...
--*/
{
    ...
    switch (g_Battle.rgPlayer[wPlayerIndex].action.ActionType)
    {
        ...
        case kBattleActionAttackMate:
            ...
            for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
            {
                if (i == wPlayerIndex) { continue; }

                if (gpGlobals->g.PlayerRoles.rgwHP[gpGlobals->rgParty[i].wPlayerRole] > 0) { break; }
            }
            ...
            if (i <= gpGlobals->wMaxPartyMemberIndex)
            {
                ...
                if (g_Battle.rgPlayer[sTarget].fDefending) { def *= 2; }
                ...
                g_Battle.rgPlayer[sTarget].pos =
                    PAL_XY(PAL_X(g_Battle.rgPlayer[sTarget].pos) - 12, PAL_Y(g_Battle.rgPlayer[sTarget].pos) - 6);
                PAL_BattleDelay(1, 0, TRUE);
        ...
        case kBattleActionCoopMagic:
            ...
            else
            {
                for (i = 1; i <= 6; i++)
                {
                    ...
                    for (j = 0; j <= gpGlobals->wMaxPartyMemberIndex; j++)
                    {
                        if ((WORD)j == wPlayerIndex) { continue; }
                ...
                {
                    if ((WORD)i == wPlayerIndex) { continue; }
            ...
            if (sTarget == -1)
            {
                ...
                for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++)
                {
                    ...
                    sDamage = PAL_CalcMagicDamage(str,
                        def,
        ...
        case kBattleActionMagic:
                ...
                gpGlobals->g.lprgMagic[wMagicNum].wType == kMagicTypeTrance)
            {
                ...
                else if (gpGlobals->g.lprgMagic[wMagicNum].wType == kMagicTypeTrance) { w = wPlayerRole; }
        ...
        case kBattleActionUseItem:
            ...
            if (g_Battle.iHidingTime < 0)
            {
                ...
                if (gpGlobals->bBattleSpeed > 1) { g_Battle.iHidingTime *= 1 + (gpGlobals->bBattleSpeed - 1) * 0.5; }
                else { g_Battle.iHidingTime *= 1.2; }
#endif
                PAL_BattleBackupScene();
                PAL_BattleMakeScene();
                PAL_BattleFadeScene();
    ...
    if (fCheckPoison)
    {
        ...
        if (fPoisoned)
        {
            PAL_BattleDelay(3, 0, TRUE);
            PAL_BattleUpdateFighters();
            if (PAL_BattleDisplayStatChange()) { PAL_BattleDelay(6, 0, TRUE); }
        }
    }
    ...
    {
        if (gpGlobals->rgPlayerStatus[wPlayerRole][i] > 0) { gpGlobals->rgPlayerStatus[wPlayerRole][i]--; }
    }
#endif
}
...
--*/
{
             ...
             g_Battle.rgEnemy[wEnemyIndex].rgwStatus[kStatusSilence] == 0)
    {
        ...
        if (g_Battle.rgEnemy[wEnemyIndex].e.wMagicFrames == 0) { PAL_BattleDelay(1, 0, FALSE); }
...
#endif
                 gpGlobals->rgPlayerStatus[wPlayerRole][kStatusConfused] == 0 && RandomLong(0, 2) == 0)
        {
            fAutoDefend                              = TRUE;
        ...
        if ((SHORT)(gpGlobals->g.lprgMagic[wMagicNum].wBaseDamage) > 0)
        {
            if (sTarget == -1)
            {
                ...
                for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
                {
                    ...
                    {
                        rgwElementalResistance[x] = 5 + PAL_GetPlayerElementalResistance(w, x) / 20;
                    }

                    sDamage = PAL_CalcMagicDamage(
                        str, def, rgwElementalResistance, 5 + PAL_GetPlayerPoisonResistance(w) / 20, wMagic);

                    sDamage /= ((g_Battle.rgPlayer[i].fDefending ? 2 : 1) *
                                   ((gpGlobals->rgPlayerStatus[w][kStatusProtect] > 0) ? 2 : 1)) +
                               (rgfMagAutoDefend[i] ? 1 : 0);

                    if (sDamage > gpGlobals->g.PlayerRoles.rgwHP[w]) { sDamage = gpGlobals->g.PlayerRoles.rgwHP[w]; }
            ...
            else
            {
                ...
                for (x = 0; x < NUM_MAGIC_ELEMENTAL; x++)
                {
                    rgwElementalResistance[x] = 5 + PAL_GetPlayerElementalResistance(wPlayerRole, x) / 20;
                }

                sDamage = PAL_CalcMagicDamage(
                    str, def, rgwElementalResistance, 5 + PAL_GetPlayerPoisonResistance(wPlayerRole) / 20, wMagic);
        ...
        if (!gpGlobals->fAutoBattle) { PAL_BattleDisplayStatChange(); }
        ...
        for (i = 0; i < 5; i++)
        {
            if (sTarget == -1)
            {
                for (x = 0; x <= gpGlobals->wMaxPartyMemberIndex; x++)
                {
                    ...
                    if (i > 0)
                    {
                        g_Battle.rgPlayer[x].pos = PAL_XY(
                            PAL_X(g_Battle.rgPlayer[x].pos) + (8 >> i), PAL_Y(g_Battle.rgPlayer[x].pos) + (4 >> i));
                    }
    ...
    else
    {
        ...
        //
        if ((PAL_IsPlayerDying(wPlayerRole) || gpGlobals->rgPlayerStatus[wPlayerRole][kStatusConfused] > 0 ||
                gpGlobals->rgPlayerStatus[wPlayerRole][kStatusSleep] > 0 ||
                gpGlobals->rgPlayerStatus[wPlayerRole][kStatusParalyzed] > 0) &&
            fAutoDefend)
        {
        ...
        else
        {
            ...
            {
                g_Battle.rgEnemy[wEnemyIndex].wCurrentFrame =
                    g_Battle.rgEnemy[wEnemyIndex].e.wIdleFrames + g_Battle.rgEnemy[wEnemyIndex].e.wMagicFrames + i - 1;
        ...
        if (!fAutoDefend)
        {
            ...
            if (gpGlobals->rgPlayerStatus[wPlayerRole][kStatusProtect]) { sDamage /= 2; }
        ...
        if (iCoverIndex != -1)
        {
            g_Battle.rgEnemy[wEnemyIndex].pos =
                PAL_XY(PAL_X(g_Battle.rgEnemy[wEnemyIndex].pos) - 10, PAL_Y(g_Battle.rgEnemy[wEnemyIndex].pos) - 8);
            g_Battle.rgPlayer[iCoverIndex].pos =
                PAL_XY(PAL_X(g_Battle.rgPlayer[iCoverIndex].pos) + 4, PAL_Y(g_Battle.rgPlayer[iCoverIndex].pos) + 2);
        }
        else
        {
            g_Battle.rgPlayer[sTarget].pos =
                PAL_XY(PAL_X(g_Battle.rgPlayer[sTarget].pos) + 8, PAL_Y(g_Battle.rgPlayer[sTarget].pos) + 4);
        }
        ...
        else if (PAL_IsPlayerDying(wPlayerRole)) { wFrameBak = 1; }

        if (iCoverIndex == -1)
        {
            g_Battle.rgPlayer[sTarget].pos =
                PAL_XY(PAL_X(g_Battle.rgPlayer[sTarget].pos) + 2, PAL_Y(g_Battle.rgPlayer[sTarget].pos) + 1);
        }
    ...
    if (!g_Battle.rgEnemy[wEnemyIndex].fDualMove)
    {
        ...
        if (PAL_BattleDisplayStatChange()) { PAL_BattleDelay(6, 0, FALSE); }
    }
    ...
    for (i = 0; i < kStatusAll; i++)
    {
        if (g_Battle.rgEnemy[wEnemyIndex].rgwStatus[i] > 0) { g_Battle.rgEnemy[wEnemyIndex].rgwStatus[i]--; }
    }
#else
    i = 0;  // do nothing
#endif
...
--*/
{
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        if (i == 4) { g_Battle.rgEnemy[wTarget].iColorShift = 6; }
    ...
    if (g_Battle.rgEnemy[wTarget].e.nStealItem > 0 && (RandomLong(0, 10) <= wStealRate || wStealRate == 0))
    {
...
--*/
{
    ...
    if (gpGlobals->g.rgObject[wMagicObjectID].magic.wFlags & kMagicFlagApplyToAll) { sTarget = -1; }
    else if (sTarget == -1) { sTarget = PAL_BattleSelectAutoTarget(); }
        ...
        wBaseDamage > 0)
    {
        if (sTarget == -1)
        {
            ...
            for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++)
            {
                ...
                sDamage = PAL_CalcMagicDamage(wBaseDamage,
                    (WORD)def,
                    g_Battle.rgEnemy[i].e.wElemResistance,
                    g_Battle.rgEnemy[i].e.wPoisonResistance,
                    wMagicObjectID);
