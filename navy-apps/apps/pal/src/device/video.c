...
--*/
{
    ...
    gpWindow = SDL_CreateWindow(
        "Pal", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, wScreenWidth, wScreenHeight, SDL_WINDOW_SHOWN);
#else
    ...
    gpScreenReal = SDL_CreateRGBSurface(SDL_SWSURFACE, 320, 200, 32, 0x00FF0000, 0x0000FF00, 0x000000FF, 0xFF000000);

    //
    // Create texture for screen.
    //
    gpTexture = SDL_CreateTexture(gpRenderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING, 320, 200);
...
#ifdef __S60_5X__
    gpScreenReal = SDL_SetVideoMode(640, 360, 8, SDL_SWSURFACE | (fFullScreen ? SDL_FULLSCREEN : 0));
#else
    gpScreenReal = SDL_SetVideoMode(320, 240, 8, SDL_SWSURFACE | (fFullScreen ? SDL_FULLSCREEN : 0));
#endif
#elif defined(GEKKO)
    gpScreenReal = SDL_SetVideoMode(640, 480, 8, SDL_SWSURFACE | (fFullScreen ? SDL_FULLSCREEN : 0));
#elif defined(PSP)
    gpScreenReal = SDL_SetVideoMode(320, 240, 8, SDL_SWSURFACE | SDL_FULLSCREEN);
#else
    gpScreenReal = SDL_SetVideoMode(320, 200, 8, SDL_HWSURFACE | SDL_RESIZABLE | (fFullScreen ? SDL_FULLSCREEN : 0));
#endif
    ...
    // Still fail?
    //
    if (gpScreenReal == NULL) { return -1; }

    //
    // Create the screen buffer and the backup screen buffer.
    //
    gpScreen = SDL_CreateRGBSurface(gpScreenReal->flags & ~SDL_HWSURFACE,
        320,
        200,
        8,
        gpScreenReal->format->Rmask,
        gpScreenReal->format->Gmask,
        gpScreenReal->format->Bmask,
        gpScreenReal->format->Amask);

    gpScreenBak = SDL_CreateRGBSurface(gpScreenReal->flags & ~SDL_HWSURFACE,
        320,
        200,
        8,
        gpScreenReal->format->Rmask,
    ...
    if (fFullScreen) { SDL_ShowCursor(FALSE); }
...
--*/
{
    if (gpScreen != NULL) { SDL_FreeSurface(gpScreen); }
    gpScreen = NULL;

    if (gpScreenBak != NULL) { SDL_FreeSurface(gpScreenBak); }
    gpScreenBak = NULL;

#if SDL_VERSION_ATLEAST(2, 0, 0)

    if (gpTouchOverlay) { SDL_DestroyTexture(gpTouchOverlay); }
    gpTouchOverlay = NULL;

#ifdef __WINPHONE__
    if (gpBackKeyMessage) { SDL_DestroyTexture(gpBackKeyMessage); }
    ...
    if (gpRenderer) { SDL_DestroyRenderer(gpRenderer); }
    gpRenderer = NULL;

    if (gpWindow) { SDL_DestroyWindow(gpWindow); }
    gpWindow = NULL;

#endif

    if (gpScreenReal != NULL) { SDL_FreeSurface(gpScreenReal); }
    gpScreenReal = NULL;
}
...
--*/
{
    ...
    if (lpRect != NULL)
    {
        ...
        if (gpTouchOverlay) { SDL_RenderCopy(gpRenderer, gpTouchOverlay, NULL, NULL); }
#ifdef __WINPHONE__
        if (gpBackKeyMessage)
        {
    ...
    else
    {
        if (g_wShakeTime != 0)
        {
            ...
            if (g_wShakeTime & 1) { srcrect.y = g_wShakeLevel; }
            else { dstrect.y = (screenRealY + g_wShakeLevel) * screenRealHeight / gpScreen->h; }
...
--*/
{
    ...
    for (i = 0; i < 256; i++) { palette->colors[i] = rgPalette[i]; }
...
--*/
{
    ...
    else { gpScreenReal = SDL_SetVideoMode(g_wInitialWidth, g_wInitialHeight, 8, flags); }
...
{
    if (gpWindow != NULL) { SDL_SetWindowTitle(gpWindow, lpszCaption); }
}
