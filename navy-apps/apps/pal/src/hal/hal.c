...
--*/
{
    ...
    if (evt.type == NDL_EVENT_TIMER) { systime = evt.data; }
...
{
    while (systime < tick) { while (PAL_PollEvent(NULL)); }
    if (systime > tick) return;
}

uint32_t SDL_GetTicks() { return systime; }
...
void SDL_FillRect(SDL_Surface* dst, SDL_Rect* dstrect, uint32_t color)
{
    ...
    for (int i = 0; i < w; i++)
        for (int j = 0; j < h; j++) { dst->pixels[(dx + i) + (dy + j) * dst->w] = color; }
...
{
    return SDL_CreateRGBSurface(flags, width, height, bpp, 0x000000ff, 0x0000ff00, 0x00ff0000, 0xff000000);
}
...
void SDL_FreeSurface(SDL_Surface* s)
{
    if (s != NULL)
    {
        ...
        if (s->pixels != NULL && s->pixels != (void*)VMEM_ADDR) { free(s->pixels); }

        free(s);
    }
}

void hal_init() { NDL_OpenDisplay(W, H); }
