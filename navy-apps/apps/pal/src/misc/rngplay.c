...
--*/
{
    ...
    if (lpBuffer == NULL || fpRngMKF == NULL || uiBufferSize == 0) { return -1; }
    ...
    if (iChunkLen != 0) { fseek(fpRngMKF, uiOffset, SEEK_SET); }
    else { return -1; }
    ...
    if ((UINT)iChunkLen > uiBufferSize) { return -2; }
...
static INT PAL_RNGBlitToSurface(INT iNumRNG, INT iNumFrame, SDL_Surface* lpDstSurface, FILE* fpRngMKF)
/*++
  Purpose:
...
--*/
{
    ...
    //
    if (lpDstSurface == NULL || iNumRNG < 0 || iNumFrame < 0) { return -1; }
