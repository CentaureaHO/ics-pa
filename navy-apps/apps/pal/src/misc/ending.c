...
--*/
{
    ...
    if (PAL_MKFDecompressChunk(buf, 320 * 200, wChunkNum, gpGlobals->f.fpFBP) <= 0) { memset(buf, 0, sizeof(buf)); }
    ...
    if (wFade)
    {
        ...
        p = SDL_CreateRGBSurface(gpScreen->flags & ~SDL_HWSURFACE,
            320,
            200,
            8,
            ...
            for (j = 0; j < 6; j++)
            {
                ...
                if (g_wCurEffectSprite != 0)
                {
                    ...
                    PAL_RLEBlitToSurface(
                        PAL_SpriteGetFrame(bufSprite, f % PAL_SpriteGetNumFrames(bufSprite)), gpScreen, PAL_XY(0, 0));
                }
...
--*/
{
    ...
    if (PAL_MKFDecompressChunk(buf, 320 * 200, wChunkNum, gpGlobals->f.fpFBP) <= 0) { return; }
    ...
    if (wScrollSpeed == 0) { wScrollSpeed = 1; }
...
--*/
{
    ...
    pUpper = SDL_CreateRGBSurface(gpScreen->flags & ~SDL_HWSURFACE,
        320,
        200,
        8,
        gpScreen->format->Rmask,
    ...
    pLower = SDL_CreateRGBSurface(gpScreen->flags & ~SDL_HWSURFACE,
        320,
        200,
        8,
        gpScreen->format->Rmask,
    ...
    for (i = 0; i < 400; i++)
    {
        ...
        PAL_RLEBlitToSurface(PAL_SpriteGetFrame(bufGirl, (SDL_GetTicks() / 50) % 4), gpScreen, PAL_XY(220, yPosGirl));
...
VOID PAL_EndingScreen(VOID)
{
    ...
    if (!SOUND_PlayCDA(2)) { RIX_Play(0x11, TRUE, 0); }
    ...
    if (!SOUND_PlayCDA(13)) { RIX_Play(9, TRUE, 0); }
