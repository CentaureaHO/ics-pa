...
static unsigned short get_loop(const void* src, unsigned int* bitptr, PYJ_1_BLOCKHEADER header)
{
...
static unsigned short get_count(const void* src, unsigned int* bitptr, PYJ_1_BLOCKHEADER header)
{
    unsigned short temp;
...
INT Decompress(LPCVOID Source, LPVOID Destination, INT DestSize)
{
    ...
    do {
        ...
        if ((node = root = (TreeNode*)malloc(sizeof(TreeNode) * (tree_len + 1))) == NULL) return -1;
        root[0].leaf  = 0;
        root[0].value = 0;
        root[0].left  = root + 1;
        root[0].right = root + 2;
...
static unsigned char data1[0x100] = {0x3f,
    0x0b,
    0x17,
    0x03,
    0x2f,
...
static int build_tree(Tree* tree)
{
    ...
    TreeNode** list;
    TreeNode*  node;
    if ((tree->list = list = (TreeNode**)malloc(sizeof(TreeNode*) * 321)) == NULL) return 0;
...
INT Decompress(LPCVOID Source, LPVOID Destination, INT DestSize)
{
    ...
    while (1)
    {
        ...
        if (val > 0xff)
        {
            ...
            unsigned char* pre;
            for (i = 0, temp = 0; i < 8; i++, ptr++) temp |= (unsigned int)bt(src, ptr) << i;
            tmp = temp & 0xff;
            for (; i < data2[tmp & 0xf] + 6; i++, ptr++) temp |= (unsigned int)bt(src, ptr) << i;
            temp >>= data2[tmp & 0xf];
