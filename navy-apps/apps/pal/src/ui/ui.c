...
--*/
{
    ...
    for (i = 0; i < 3; i++)
    {
        for (j = 0; j < 3; j++) { rglpBorderBitmap[i][j] = PAL_SpriteGetFrame(gpSpriteUI, i * 3 + j + iStyle * 9); }
    }
    ...
    if (fSaveScreen)
    {
        ...
        //
        save = SDL_CreateRGBSurface(gpScreen->flags,
            rect.w,
            rect.h,
            8,
...
WORD PAL_ReadMenu(LPITEMCHANGED_CALLBACK lpfnMenuItemChanged, LPMENUITEM rgMenuItem, INT nMenuItem, WORD wDefaultItem,
    BYTE bLabelColor)
/*++
  Purpose:
...
--*/
{
    ...
    for (i = 0; i < nMenuItem; i++)
    {
        ...
        {
            if (i == wCurrentItem) { bColor = MENUITEM_COLOR_SELECTED_INACTIVE; }
    ...
    if (lpfnMenuItemChanged != NULL) { (*lpfnMenuItemChanged)(rgMenuItem[wDefaultItem].wValue); }
    ...
    while (TRUE)
    {
        ...
        if (g_InputState.dwKeyPress & (kKeyDown | kKeyRight))
        {
            ...
            if (wCurrentItem >= nMenuItem) { wCurrentItem = 0; }
            ...
            if (lpfnMenuItemChanged != NULL) { (*lpfnMenuItemChanged)(rgMenuItem[wCurrentItem].wValue); }
        }
        else if (g_InputState.dwKeyPress & (kKeyUp | kKeyLeft))
        {
            ...
            if (wCurrentItem > 0) { wCurrentItem--; }
            else { wCurrentItem = nMenuItem - 1; }
...
--*/
{
    ...
    for (i = 0; i < 10; i++) { rglpBitmap[i] = PAL_SpriteGetFrame(gpSpriteUI, (UINT)x + i); }
...
--*/
{
    while (lpObjectDesc != NULL)
    {
        if (lpObjectDesc->wObjectID == wObjectID) { return lpObjectDesc->lpDesc; }
