...
--*/
{
    ...
    PAL_MKFReadChunk((LPBYTE)(g_TextLib.lpMsgOffset), i * sizeof(DWORD), 3, gpGlobals->f.fpSSS);
...
--*/
{
    ...
    if (wNumWord >= PAL_ADDITIONAL_WORD_FIRST) { return g_rgszAdditionalWords[wNumWord - PAL_ADDITIONAL_WORD_FIRST]; }

    if (wNumWord >= g_TextLib.nWords) { return NULL; }
    ...
    if ((strlen(buf) & 1) != 0 && buf[strlen(buf) - 1] == '1') { buf[strlen(buf) - 1] = '\0'; }
...
--*/
{
    ...
    if (wNumMsg >= g_TextLib.nMsgs) { return NULL; }
...
#ifdef PAL_WIN95
        urect.w++;
        if (urect.x + urect.w > 320) { urect.w = 320 - urect.x; }
#endif
...
--*/
{
    ...
    if (bFontColor != 0) { g_TextLib.bCurrentFontColor = bFontColor; }
...
--*/
{
    ...
    if (g_TextLib.bDialogPosition != kDialogCenterWindow && g_TextLib.bDialogPosition != kDialogCenter)
    {
        //
        // show the icon
        //
    ...
    while (TRUE)
    {
        ...
        if (g_TextLib.bDialogPosition != kDialogCenterWindow && g_TextLib.bDialogPosition != kDialogCenter)
        {
            ...
            //
            t = palette[0xF9];
            for (i = 0xF9; i < 0xFE; i++) { palette[i] = palette[i + 1]; }
            palette[0xFE] = t;
            ...
            while (lpszText != NULL && *lpszText != '\0')
            {
                switch (*lpszText)
                {
                    case '-':
                        ...
                        else { g_TextLib.bCurrentFontColor = FONT_COLOR_CYAN; }
                        lpszText++;
                        break;
#ifndef PAL_WIN95
                    case '\'':
                        ...
                        else { g_TextLib.bCurrentFontColor = FONT_COLOR_RED; }
                        lpszText++;
                        break;
#endif
                    case '\"':
                        ...
                        else { g_TextLib.bCurrentFontColor = FONT_COLOR_YELLOW; }
                        lpszText++;
                        break;
...
--*/
{
    if (g_TextLib.nCurrentDialogLine > 0 && fWaitForKey) { PAL_DialogWaitForKey(); }
