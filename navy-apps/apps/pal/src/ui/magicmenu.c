...
--*/
{
    ...
    // Check for inputs
    //
    if (g_InputState.dwKeyPress & kKeyUp) { g_iCurrentItem -= 3; }
    else if (g_InputState.dwKeyPress & kKeyDown) { g_iCurrentItem += 3; }
    else if (g_InputState.dwKeyPress & kKeyLeft) { g_iCurrentItem--; }
    else if (g_InputState.dwKeyPress & kKeyRight) { g_iCurrentItem++; }
    else if (g_InputState.dwKeyPress & kKeyPgUp) { g_iCurrentItem -= 3 * 5; }
    else if (g_InputState.dwKeyPress & kKeyPgDn) { g_iCurrentItem += 3 * 5; }
    else if (g_InputState.dwKeyPress & kKeyMenu) { return 0; }

    //
    // Make sure the current menu item index is in bound
    //
    if (g_iCurrentItem < 0) { g_iCurrentItem = 0; }
    else if (g_iCurrentItem >= g_iNumMagic) { g_iCurrentItem = g_iNumMagic - 1; }
    ...
    if (gpGlobals->lpObjectDesc == NULL)
    {
        ...
        PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH), gpScreen, PAL_XY(260, 14));
        PAL_DrawNumber(rgMagicItem[g_iCurrentItem].wMP, 4, PAL_XY(230, 14), kNumColorYellow, kNumAlignRight);
    ...
    for (j = 0; j < 5; j++)
    {
        for (k = 0; k < 3; k++)
        {
            ...
            if (i == g_iCurrentItem)
            {
                if (rgMagicItem[i].fEnabled) { bColor = MENUITEM_COLOR_SELECTED; }
                else { bColor = MENUITEM_COLOR_SELECTED_INACTIVE; }
            }
            else if (!rgMagicItem[i].fEnabled) { bColor = MENUITEM_COLOR_INACTIVE; }

            //
            // Draw the text
            //
            PAL_DrawText(PAL_GetWord(rgMagicItem[i].wMagic), PAL_XY(35 + k * 87, 54 + j * 18), bColor, TRUE, FALSE);
            ...
            if (i == g_iCurrentItem)
            {
                PAL_RLEBlitToSurface(
                    PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_CURSOR), gpScreen, PAL_XY(60 + k * 87, 64 + j * 18));
            }
    ...
    if (g_InputState.dwKeyPress & kKeySearch)
    {
        if (rgMagicItem[g_iCurrentItem].fEnabled)
        {
            ...
            PAL_DrawText(
                PAL_GetWord(rgMagicItem[g_iCurrentItem].wMagic), PAL_XY(j, k), MENUITEM_COLOR_CONFIRMED, FALSE, TRUE);
...
--*/
{
    ...
    for (i = 0; i < MAX_PLAYER_MAGICS; i++)
    {
        ...
        if (w != 0)
        {
            ...
            if (rgMagicItem[g_iNumMagic].wMP > g_wPlayerMP) { rgMagicItem[g_iNumMagic].fEnabled = FALSE; }

            w = gpGlobals->g.rgObject[rgMagicItem[g_iNumMagic].wMagic].magic.wFlags;
            if (fInBattle)
            {
                if (!(w & kMagicFlagUsableInBattle)) { rgMagicItem[g_iNumMagic].fEnabled = FALSE; }
            }
            else
            {
                if (!(w & kMagicFlagUsableOutsideBattle)) { rgMagicItem[g_iNumMagic].fEnabled = FALSE; }
            }
...
--*/
{
    ...
    while (TRUE)
    {
        ...
        {
            PAL_PlayerInfoBox(PAL_XY(w, 165), gpGlobals->rgParty[i].wPlayerRole, 100, TIMEMETER_COLOR_DEFAULT, FALSE);
            w += 78;
        }
