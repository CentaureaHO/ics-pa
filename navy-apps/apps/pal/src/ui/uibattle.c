...
--*/
{
    ...
    {
        if (gpGlobals->rgParty[iPartyIndex].wPlayerRole == wPlayerRole) { break; }
    }
    ...
    PAL_RLEBlitToSurface(
        PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH), gpScreen, PAL_XY(PAL_X(pos) + 49, PAL_Y(pos) + 6));
    PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwMaxHP[wPlayerRole],
        4,
        PAL_XY(PAL_X(pos) + 47, PAL_Y(pos) + 8),
        kNumColorYellow,
        kNumAlignRight);
    PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole],
        4,
        PAL_XY(PAL_X(pos) + 26, PAL_Y(pos) + 5),
        kNumColorYellow,
        kNumAlignRight);

    PAL_RLEBlitToSurface(
        PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH), gpScreen, PAL_XY(PAL_X(pos) + 49, PAL_Y(pos) + 22));
    PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwMaxMP[wPlayerRole],
        4,
        PAL_XY(PAL_X(pos) + 47, PAL_Y(pos) + 24),
        kNumColorCyan,
        kNumAlignRight);
    PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwMP[wPlayerRole],
        4,
        PAL_XY(PAL_X(pos) + 26, PAL_Y(pos) + 21),
        kNumColorCyan,
        kNumAlignRight);
#else
    PAL_RLEBlitToSurface(
        PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH), gpScreen, PAL_XY(PAL_X(pos) + 49, PAL_Y(pos) + 14));
    PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwMaxHP[wPlayerRole],
        4,
        PAL_XY(PAL_X(pos) + 47, PAL_Y(pos) + 16),
        kNumColorYellow,
        kNumAlignRight);
    PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole],
        4,
        PAL_XY(PAL_X(pos) + 26, PAL_Y(pos) + 13),
        kNumColorYellow,
        kNumAlignRight);

    PAL_RLEBlitToSurface(
        PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH), gpScreen, PAL_XY(PAL_X(pos) + 49, PAL_Y(pos) + 24));
    PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwMaxMP[wPlayerRole],
        4,
        PAL_XY(PAL_X(pos) + 47, PAL_Y(pos) + 26),
        kNumColorCyan,
        kNumAlignRight);
    PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwMP[wPlayerRole],
        4,
        PAL_XY(PAL_X(pos) + 26, PAL_Y(pos) + 23),
        kNumColorCyan,
        kNumAlignRight);
#endif
...
--*/
{
    ...
    switch (ActionType)
    {
        ...
        case kBattleUIActionMagic:
            if (gpGlobals->rgPlayerStatus[wPlayerRole][kStatusSilence] != 0) { return FALSE; }
            break;

        case kBattleUIActionCoopMagic:
            if (gpGlobals->wMaxPartyMemberIndex == 0) { return FALSE; }
            for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
            {
                ...
                if (gpGlobals->g.PlayerRoles.rgwHP[w] < gpGlobals->g.PlayerRoles.rgwMaxHP[w] / 5 ||
                    ...
                    gpGlobals->rgPlayerStatus[w][kStatusSilence] != 0 || g_Battle.rgPlayer[i].flTimeMeter < 100 ||
...
--*/
{
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        if (i == wCurrentItem)
        {
            if (fConfirmed) { bColor = MENUITEM_COLOR_CONFIRMED; }
            else { bColor = MENUITEM_COLOR_SELECTED; }
        }
...
--*/
{
    ...
    {
        g_iCurMiscMenuItem--;
        if (g_iCurMiscMenuItem < 0) { g_iCurMiscMenuItem = 4; }
    }
    else if (g_InputState.dwKeyPress & (kKeyDown | kKeyRight))
    {
        g_iCurMiscMenuItem++;
        if (g_iCurMiscMenuItem > 4) { g_iCurMiscMenuItem = 0; }
    }
    else if (g_InputState.dwKeyPress & kKeySearch) { return g_iCurMiscMenuItem + 1; }
    else if (g_InputState.dwKeyPress & kKeyMenu) { return 0; }
...
--*/
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        if (i == g_iCurSubMenuItem) { bColor = MENUITEM_COLOR_SELECTED; }
    ...
    // Process inputs
    //
    if (g_InputState.dwKeyPress & (kKeyUp | kKeyLeft)) { g_iCurSubMenuItem = 0; }
    else if (g_InputState.dwKeyPress & (kKeyDown | kKeyRight)) { g_iCurSubMenuItem = 1; }
    else if (g_InputState.dwKeyPress & kKeySearch) { return g_iCurSubMenuItem + 1; }
    else if (g_InputState.dwKeyPress & kKeyMenu) { return 0; }
...
--*/
{
    ...
    //
    if (gpGlobals->rgPlayerStatus[w][kStatusPuppet] == 0 && gpGlobals->rgPlayerStatus[w][kStatusSleep] == 0 &&
        gpGlobals->rgPlayerStatus[w][kStatusConfused] == 0 && !g_Battle.UI.fAutoAttack && !gpGlobals->fAutoBattle)
    {
...
--*/
{
    ...
    if (wSelectedItem != 0xFFFF)
    {
        ...
        else { g_Battle.UI.MenuState = kBattleMenuMain; }
    }
}
...
--*/
{
    ...
    if (g_Battle.UI.fAutoAttack && !gpGlobals->fAutoBattle)
    {
        ...
        //
        if (g_InputState.dwKeyPress & kKeyMenu) { g_Battle.UI.fAutoAttack = FALSE; }
        else { PAL_DrawText(PAL_GetWord(BATTLEUI_LABEL_AUTO), PAL_XY(280, 10), MENUITEM_COLOR_CONFIRMED, TRUE, FALSE); }
    }
    ...
    if (gpGlobals->fAutoBattle)
    {
        ...
        if (g_Battle.UI.state != kBattleUIWait)
        {
            ...
            else
            {
                ...
                if (gpGlobals->g.rgObject[w].magic.wFlags & kMagicFlagApplyToAll) { g_Battle.UI.wSelectedIndex = -1; }
                else { g_Battle.UI.wSelectedIndex = PAL_BattleSelectAutoTarget(); }
            }
...
#ifdef PAL_CLASSIC
    if (g_Battle.Phase == kBattlePhasePerformAction) { goto end; }
...
#ifndef PAL_CLASSIC
            if (gpGlobals->rgPlayerStatus[wPlayerRole][kStatusHaste] > 0) { j = TIMEMETER_COLOR_HASTE; }
            else if (gpGlobals->rgPlayerStatus[wPlayerRole][kStatusSlow] > 0) { j = TIMEMETER_COLOR_SLOW; }
#endif
    ...
    if (g_Battle.UI.state != kBattleUIWait)
    {
        ...
        if (gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole] == 0 && gpGlobals->rgPlayerStatus[wPlayerRole][kStatusPuppet])
        {
            g_Battle.UI.wActionType = kBattleActionAttack;
        ...
        if (s_iFrame & 1) { i = SPRITENUM_BATTLE_ARROW_CURRENTPLAYER; }
    ...
    switch (g_Battle.UI.state)
    {
        ...
        case kBattleUISelectMove:
            ...
            //
            {
                ...
                {
                    if (g_InputState.dir == kDirNorth) { g_Battle.UI.wSelectedAction = 0; }
                    else if (g_InputState.dir == kDirSouth) { g_Battle.UI.wSelectedAction = 3; }
                    else if (g_InputState.dir == kDirWest)
                    {
                        if (PAL_BattleUIIsActionValid(kBattleUIActionMagic)) { g_Battle.UI.wSelectedAction = 1; }
                    }
                    else if (g_InputState.dir == kDirEast)
                    {
                        if (PAL_BattleUIIsActionValid(kBattleUIActionCoopMagic)) { g_Battle.UI.wSelectedAction = 2; }
                    }
                }
                ...
                for (i = 0; i < 4; i++)
                {
                    ...
                    {
                        PAL_RLEBlitToSurface(
                            PAL_SpriteGetFrame(gpSpriteUI, rgItems[i].iSpriteNum), gpScreen, rgItems[i].pos);
                    }
                    else if (PAL_BattleUIIsActionValid(rgItems[i].action))
                    {
                        PAL_RLEBlitMonoColor(
                            PAL_SpriteGetFrame(gpSpriteUI, rgItems[i].iSpriteNum), gpScreen, rgItems[i].pos, 0, -4);
                    }
                    else
                    {
                        PAL_RLEBlitMonoColor(
                            PAL_SpriteGetFrame(gpSpriteUI, rgItems[i].iSpriteNum), gpScreen, rgItems[i].pos, 0x10, -4);
                    }
                }
                ...
                switch (g_Battle.UI.MenuState)
                {
                    case kBattleMenuMain:
                        ...
                        else if (g_InputState.dwKeyPress & kKeyRepeat) { PAL_BattleCommitAction(TRUE); }
#ifdef PAL_CLASSIC
        ...
        case kBattleUISelectTargetEnemy:
...
#endif
            if (g_Battle.UI.wSelectedIndex > x) { g_Battle.UI.wSelectedIndex = x; }

            for (i = 0; i <= x; i++)
            {
                if (g_Battle.rgEnemy[g_Battle.UI.wSelectedIndex].wObjectID != 0) { break; }
            ...
            if (g_InputState.dwKeyPress & kKeyMenu) { g_Battle.UI.state = kBattleUISelectMove; }
            ...
            else if (g_InputState.dwKeyPress & (kKeyLeft | kKeyDown))
            {
                if (g_Battle.UI.wSelectedIndex != 0)
                {
                    g_Battle.UI.wSelectedIndex--;
                    while (
                        g_Battle.UI.wSelectedIndex != 0 && g_Battle.rgEnemy[g_Battle.UI.wSelectedIndex].wObjectID == 0)
                    {
                        g_Battle.UI.wSelectedIndex--;
                    }
                }
            ...
            else if (g_InputState.dwKeyPress & (kKeyRight | kKeyUp))
            {
                if (g_Battle.UI.wSelectedIndex < x)
                {
                    g_Battle.UI.wSelectedIndex++;
                    while (
                        g_Battle.UI.wSelectedIndex < x && g_Battle.rgEnemy[g_Battle.UI.wSelectedIndex].wObjectID == 0)
                    {
                        g_Battle.UI.wSelectedIndex++;
                    }
                }
        ...
        case kBattleUISelectTargetPlayer:
            ...
            if (s_iFrame & 1) { j = SPRITENUM_BATTLE_ARROW_SELECTEDPLAYER_RED; }
            ...
            if (g_InputState.dwKeyPress & kKeyMenu) { g_Battle.UI.state = kBattleUISelectMove; }
            else if (g_InputState.dwKeyPress & kKeySearch) { PAL_BattleCommitAction(FALSE); }
            else if (g_InputState.dwKeyPress & (kKeyLeft | kKeyDown))
            {
                if (g_Battle.UI.wSelectedIndex != 0) { g_Battle.UI.wSelectedIndex--; }
                else { g_Battle.UI.wSelectedIndex = gpGlobals->wMaxPartyMemberIndex; }
            }
            else if (g_InputState.dwKeyPress & (kKeyRight | kKeyUp))
            {
                if (g_Battle.UI.wSelectedIndex < gpGlobals->wMaxPartyMemberIndex) { g_Battle.UI.wSelectedIndex++; }
                else { g_Battle.UI.wSelectedIndex = 0; }
            }
    ...
    for (i = 0; i < BATTLEUI_MAX_SHOWNUM; i++)
    {
        if (g_Battle.UI.rgShowNum[i].wNum > 0)
        {
            ...
            {
                PAL_DrawNumber(g_Battle.UI.rgShowNum[i].wNum,
                    5,
