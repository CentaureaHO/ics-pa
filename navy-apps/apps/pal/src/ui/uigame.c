...
--*/
{
    ...
    while (TRUE)
    {
        ...
        else
        {
            ...
            if (wItemSelected != MENUITEM_VALUE_CANCELLED) { break; }
            wDefaultItem = 1;
        }
    }
...
--*/
{
    ...
    for (i = 1; i <= 5; i++)
    {
        ...
        // Draw the number
        //
        PAL_DrawNumber((UINT)wSavedTimes, 4, PAL_XY(270, 38 * i - 17), kNumColorYellow, kNumAlignRight);
    }
    ...
    // Delete the boxes
    //
    for (i = 0; i < 5; i++) { PAL_DeleteBox(rgpBox[i]); }
...
--*/
{
    ...
    // Create the boxes
    //
    for (i = 0; i < 2; i++) { rgpBox[i] = PAL_CreateSingleLineBox(PAL_XY(130 + 75 * i, 100), 2, TRUE); }
    ...
    // Delete the boxes
    //
    for (i = 0; i < 2; i++) { PAL_DeleteBox(rgpBox[i]); }
...
--*/
{
    ...
    if (wReturnValue == MENUITEM_VALUE_CANCELLED) { return fEnabled; }
...
--*/
{
    ...
    if (wReturnValue != MENUITEM_VALUE_CANCELLED) { gpGlobals->bBattleSpeed = wReturnValue; }
}
...
--*/
{
...
#ifdef PAL_CLASSIC
    wReturnValue =
        PAL_ReadMenu(PAL_SystemMenu_OnItemChange, rgSystemMenuItem, 5, gpGlobals->iCurSystemMenuItem, MENUITEM_COLOR);
#else
    wReturnValue =
        PAL_ReadMenu(PAL_SystemMenu_OnItemChange, rgSystemMenuItem, 6, gpGlobals->iCurSystemMenuItem, MENUITEM_COLOR);
#endif
    ...
    switch (wReturnValue)
    {
            ...
            if (iSlot != MENUITEM_VALUE_CANCELLED)
            {
                ...
                for (i = 1; i <= 5; i++)
                {
                    ...
                    if (fp != NULL)
                    {
                        ...
                        fclose(fp);
                        wSavedTimes = SWAP16(wSavedTimes);
                        if ((int)wSavedTimes > iSavedTimes) { iSavedTimes = wSavedTimes; }
                    }
                }
        ...
        case 3:
            ...
            if (g_fUseMidi)
            {
                if (g_fNoMusic) { PAL_PlayMUS(0, FALSE, 0); }
                else { PAL_PlayMUS(gpGlobals->wNumMusic, TRUE, 0); }
            }
#endif
            break;
...
--*/
{
    ...
    {
        PAL_PlayerInfoBox(PAL_XY(y, 165), gpGlobals->rgParty[i].wPlayerRole, 100, TIMEMETER_COLOR_DEFAULT, TRUE);
        y += 78;
    }
    ...
    for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
    {
        ...
        rgMenuItem[i].fEnabled = (gpGlobals->g.PlayerRoles.rgwHP[gpGlobals->rgParty[i].wPlayerRole] > 0);
    ...
    if (w == MENUITEM_VALUE_CANCELLED) { return; }
    ...
    while (TRUE)
    {
        ...
        else
        {
            ...
            while (wPlayer != MENUITEM_VALUE_CANCELLED)
            {
                ...
                {
                    PAL_PlayerInfoBox(
                        PAL_XY(y, 165), gpGlobals->rgParty[i].wPlayerRole, 100, TIMEMETER_COLOR_DEFAULT, TRUE);
                    y += 78;
                }
                ...
                PAL_RLEBlitToSurface(
                    PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_CURSOR), gpScreen, PAL_XY(rect.x, rect.y));
                ...
                while (TRUE)
                {
                    ...
                    else if (g_InputState.dwKeyPress & kKeySearch)
                    {
                        gpGlobals->g.rgObject[wMagic].magic.wScriptOnUse = PAL_RunTriggerScript(
                            gpGlobals->g.rgObject[wMagic].magic.wScriptOnUse, gpGlobals->rgParty[wPlayer].wPlayerRole);
...
--*/
{
    ...
    {
        wReturnValue =
            PAL_ReadMenu(PAL_InGameMenu_OnItemChange, rgMainMenuItem, 4, gpGlobals->iCurMainMenuItem, MENUITEM_COLOR);

        if (wReturnValue == MENUITEM_VALUE_CANCELLED) { break; }
...
--*/
{
    ...
    while (iCurrent >= 0 && iCurrent <= gpGlobals->wMaxPartyMemberIndex)
    {
        ...
        //
        PAL_DrawNumber(
            gpGlobals->Exp.rgPrimaryExp[iPlayerRole].wExp, 5, PAL_XY(58, 6), kNumColorYellow, kNumAlignRight);
        PAL_DrawNumber(gpGlobals->g.rgLevelUpExp[gpGlobals->g.PlayerRoles.rgwLevel[iPlayerRole]],
            ...
            kNumAlignRight);
        PAL_DrawNumber(
            gpGlobals->g.PlayerRoles.rgwLevel[iPlayerRole], 2, PAL_XY(54, 35), kNumColorYellow, kNumAlignRight);
        PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH), gpScreen, PAL_XY(65, 58));
        PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH), gpScreen, PAL_XY(65, 80));
        PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwHP[iPlayerRole], 4, PAL_XY(42, 56), kNumColorYellow, kNumAlignRight);
        PAL_DrawNumber(
            gpGlobals->g.PlayerRoles.rgwMaxHP[iPlayerRole], 4, PAL_XY(63, 61), kNumColorBlue, kNumAlignRight);
        PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwMP[iPlayerRole], 4, PAL_XY(42, 78), kNumColorYellow, kNumAlignRight);
        PAL_DrawNumber(
            gpGlobals->g.PlayerRoles.rgwMaxMP[iPlayerRole], 4, PAL_XY(63, 83), kNumColorBlue, kNumAlignRight);
        ...
        for (i = 0; i < MAX_PLAYER_EQUIPMENTS; i++)
        {
            ...
            // Draw the image
            //
            if (PAL_MKFReadChunk(bufImage, 16384, gpGlobals->g.rgObject[w].item.wBitmap, gpGlobals->f.fpBALL) > 0)
            {
                PAL_RLEBlitToSurface(bufImage, gpScreen, PAL_XY(rgEquipPos[i][0], rgEquipPos[i][1]));
            }
        ...
        for (i = 0; i < MAX_POISONS; i++)
        {
            ...
            {
                PAL_DrawText(
                    PAL_GetWord(w), PAL_XY(185, y), (BYTE)(gpGlobals->g.rgObject[w].poison.wColor + 10), TRUE, FALSE);
...
--*/
{
    ...
    while (TRUE)
    {
        if (wSelectedPlayer > gpGlobals->wMaxPartyMemberIndex) { wSelectedPlayer = 0; }
        ...
        for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
        {
            if (i == wSelectedPlayer) { bColor = bSelectedColor; }
            else { bColor = MENUITEM_COLOR; }
        ...
        if (i > 0)
        {
            ...
            //
            if (PAL_MKFReadChunk(bufImage, 2048, gpGlobals->g.rgObject[wItemToUse].item.wBitmap, gpGlobals->f.fpBALL) >
                0)
            {
        ...
        while (TRUE)
        {
            ...
            if (SDL_GetTicks() > dwColorChangeTime)
            {
                ...
                PAL_DrawText(
                    ...
                    PAL_XY(125, 16 + 20 * wSelectedPlayer),
                    bSelectedColor,
                    FALSE,
                    TRUE);
            }
        ...
        if (i <= 0) { return MENUITEM_VALUE_CANCELLED; }

        if (g_InputState.dwKeyPress & (kKeyUp | kKeyLeft)) { wSelectedPlayer--; }
        else if (g_InputState.dwKeyPress & (kKeyDown | kKeyRight))
        {
            if (wSelectedPlayer < gpGlobals->wMaxPartyMemberIndex) { wSelectedPlayer++; }
        }
        else if (g_InputState.dwKeyPress & kKeyMenu) { break; }
        else if (g_InputState.dwKeyPress & kKeySearch) { return gpGlobals->rgParty[wSelectedPlayer].wPlayerRole; }
    }
...
--*/
{
    ...
    if (PAL_MKFReadChunk(bufImage, 2048, gpGlobals->g.rgObject[wCurrentItem].item.wBitmap, gpGlobals->f.fpBALL) > 0)
    {
    ...
    {
        if (gpGlobals->rgInventory[i].wItem == 0) { break; }
...
--*/
{
    ...
    {
        if (gpGlobals->g.lprgStore[wStoreNum].rgwItems[i] == 0) { break; }
    ...
    while (TRUE)
    {
        ...
        if (w == MENUITEM_VALUE_CANCELLED) { break; }
...
--*/
{
    ...
    if (gpGlobals->g.rgObject[wCurrentItem].item.wFlags & kItemFlagSellable)
    {
        ...
        PAL_DrawNumber(
            gpGlobals->g.rgObject[wCurrentItem].item.wPrice / 2, 6, PAL_XY(269, 164), kNumColorYellow, kNumAlignRight);
    }
}
...
--*/
{
    ...
    while (TRUE)
    {
        ...
        if (PAL_ConfirmMenu())
        {
            if (PAL_AddItemToInventory(w, -1)) { gpGlobals->dwCash += gpGlobals->g.rgObject[w].item.wPrice / 2; }
        }
    }
}
...
--*/
{
    ...
    while (TRUE)
    {
        ...
        //
        if (PAL_MKFReadChunk(bufImage, 2048, gpGlobals->g.rgObject[wItem].item.wBitmap, gpGlobals->f.fpBALL) > 0)
        {
        ...
        for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
        {
            ...
            if (iCurrentPlayer == i)
            {
                ...
                else { bColor = MENUITEM_COLOR_SELECTED_INACTIVE; }
            }
            else
            {
                if (gpGlobals->g.rgObject[wItem].item.wFlags & (kItemFlagEquipableByPlayerRole_First << w))
                ...
                else { bColor = MENUITEM_COLOR_INACTIVE; }
            }

            PAL_DrawText(
                PAL_GetWord(gpGlobals->g.PlayerRoles.rgwName[w]), PAL_XY(15, 108 + 18 * i), bColor, TRUE, FALSE);
        }
            ...
            if (SDL_GetTicks() > dwColorChangeTime)
            {
                ...
                if (gpGlobals->g.rgObject[wItem].item.wFlags & (kItemFlagEquipableByPlayerRole_First << w))
                {
                    PAL_DrawText(PAL_GetWord(gpGlobals->g.PlayerRoles.rgwName[w]),
                        PAL_XY(15, 108 + 18 * iCurrentPlayer),
                        bSelectedColor,
                        TRUE,
                        TRUE);
                }
        ...
        {
            iCurrentPlayer--;
            if (iCurrentPlayer < 0) { iCurrentPlayer = 0; }
        }
        else if (g_InputState.dwKeyPress & (kKeyDown | kKeyRight))
        {
            iCurrentPlayer++;
            if (iCurrentPlayer > gpGlobals->wMaxPartyMemberIndex) { iCurrentPlayer = gpGlobals->wMaxPartyMemberIndex; }
        }
        else if (g_InputState.dwKeyPress & kKeyMenu) { return; }
        else if (g_InputState.dwKeyPress & kKeySearch)
        {
            w = gpGlobals->rgParty[iCurrentPlayer].wPlayerRole;
            ...
            if (gpGlobals->g.rgObject[wItem].item.wFlags & (kItemFlagEquipableByPlayerRole_First << w))
            {
                ...
                //
                gpGlobals->g.rgObject[wItem].item.wScriptOnEquip = PAL_RunTriggerScript(
                    gpGlobals->g.rgObject[wItem].item.wScriptOnEquip, gpGlobals->rgParty[iCurrentPlayer].wPlayerRole);
            }
        }
    }
}
