...
--*/
{
    ...
    // Process input
    //
    if (g_InputState.dwKeyPress & kKeyUp) { gpGlobals->iCurInvMenuItem -= 3; }
    else if (g_InputState.dwKeyPress & kKeyDown) { gpGlobals->iCurInvMenuItem += 3; }
    else if (g_InputState.dwKeyPress & kKeyLeft) { gpGlobals->iCurInvMenuItem--; }
    else if (g_InputState.dwKeyPress & kKeyRight) { gpGlobals->iCurInvMenuItem++; }
    else if (g_InputState.dwKeyPress & kKeyPgUp) { gpGlobals->iCurInvMenuItem -= 3 * 7; }
    else if (g_InputState.dwKeyPress & kKeyPgDn) { gpGlobals->iCurInvMenuItem += 3 * 7; }
    else if (g_InputState.dwKeyPress & kKeyMenu) { return 0; }

    //
    // Make sure the current menu item index is in bound
    //
    if (gpGlobals->iCurInvMenuItem < 0) { gpGlobals->iCurInvMenuItem = 0; }
    else if (gpGlobals->iCurInvMenuItem >= g_iNumInventory) { gpGlobals->iCurInvMenuItem = g_iNumInventory - 1; }
        ...
        for (k = 0; k < 3; k++)
        {
            ...
            if (i == gpGlobals->iCurInvMenuItem)
            {
                ...
                else
                {
                    ...
                    //
                    if (gpGlobals->rgInventory[i].nAmount == 0) { bColor = MENUITEM_COLOR_EQUIPPEDITEM; }
            ...
            else if (gpGlobals->rgInventory[i].nAmount == 0) { bColor = MENUITEM_COLOR_EQUIPPEDITEM; }

            //
            // Draw the text
            //
            PAL_DrawText(PAL_GetWord(wObject), PAL_XY(15 + k * 100, 12 + j * 18), bColor, TRUE, FALSE);
            ...
            if (i == gpGlobals->iCurInvMenuItem)
            {
                PAL_RLEBlitToSurface(
                    PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_CURSOR), gpScreen, PAL_XY(40 + k * 100, 22 + j * 18));
            }
    ...
    //
    PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_ITEMBOX), gpScreen, PAL_XY(5, 140));
    ...
    {
        if (PAL_MKFReadChunk(bufImage, 2048, gpGlobals->g.rgObject[wObject].item.wBitmap, gpGlobals->f.fpBALL) > 0)
        {
            wPrevImageIndex = gpGlobals->g.rgObject[wObject].item.wBitmap;
        }
        else { wPrevImageIndex = 0xFFFF; }
    }

    if (wPrevImageIndex != 0xFFFF) { PAL_RLEBlitToSurface(bufImage, gpScreen, PAL_XY(12, 148)); }
    ...
    if (!g_fNoDesc)
    {
        ...
        while (wScript && gpGlobals->g.lprgScriptEntry[wScript].wOperation != 0)
        {
            ...
            else { wScript = PAL_RunAutoScript(wScript, 0); }
        }
    }
#endif
    ...
    if (g_InputState.dwKeyPress & kKeySearch)
    {
                ...
                (SHORT)gpGlobals->rgInventory[gpGlobals->iCurInvMenuItem].nAmountInUse)
        {
            if (gpGlobals->rgInventory[gpGlobals->iCurInvMenuItem].nAmount > 0)
            {
                ...
                PAL_DrawText(
                    PAL_GetWord(wObject), PAL_XY(15 + k * 100, 12 + j * 18), MENUITEM_COLOR_CONFIRMED, FALSE, FALSE);
            }
...
--*/
{
    ...
    //
    g_iNumInventory = 0;
    while (g_iNumInventory < MAX_INVENTORY && gpGlobals->rgInventory[g_iNumInventory].wItem != 0) { g_iNumInventory++; }
...
--*/
{
    ...
    while (TRUE)
    {
        if (lpfnMenuItemChanged == NULL) { PAL_MakeScene(); }
