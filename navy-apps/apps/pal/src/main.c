...
#endif
    if (e != 0) { TerminateOnError("Could not initialize Video: %d.\n", e); }

    Log("VIDEO_Init success");
    SDL_WM_SetCaption("Loading...", NULL);

    e = PAL_InitGlobals();
    if (e != 0) { TerminateOnError("Could not initialize global data: %d.\n", e); }
    Log("PAL_InitGolbals success");

    e = PAL_InitFont();
    if (e != 0) { TerminateOnError("Could not load fonts: %d.\n", e); }
    Log("PAL_InitFont success");

    e = PAL_InitUI();
    if (e != 0) { TerminateOnError("Could not initialize UI subsystem: %d.\n", e); }
    Log("PAL_InitUI success");

    e = PAL_InitText();
    if (e != 0) { TerminateOnError("Could not initialize text subsystem: %d.\n", e); }
...
--*/
{
    ...
    // Create the surfaces
    //
    lpBitmapDown = SDL_CreateRGBSurface(gpScreen->flags,
        320,
        200,
        8,
    ...
    lpBitmapUp   = SDL_CreateRGBSurface(gpScreen->flags,
        320,
        200,
        8,
        gpScreen->format->Rmask,
    ...
    while (TRUE)
    {
        ...
        for (i = 0; i < 9; i++)
        {
            LPCBITMAPRLE lpFrame =
                PAL_SpriteGetFrame(lpSpriteCrane, cranepos[i][2] = (cranepos[i][2] + (iCraneFrame & 1)) % 8);
    ...
    if (!fUseCD) { PAL_PlayMUS(0, FALSE, 1); }
