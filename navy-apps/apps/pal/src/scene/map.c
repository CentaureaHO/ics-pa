...
--*/
{
    ...
    //
    if (iMapNum >= PAL_MKFGetChunkCount(fpMapMKF) || iMapNum >= PAL_MKFGetChunkCount(fpGopMKF) || iMapNum <= 0)
    {
        return NULL;
    }
...
--*/
{
    ...
    //
    if (lpMap == NULL) { return; }

    //
    // Free the tile bitmaps.
    //
    if (lpMap->pTileSprite != NULL) { free(lpMap->pTileSprite); }
...
--*/
{
    ...
    //
    if (x >= 64 || y >= 128 || h > 1 || lpMap == NULL) { return NULL; }
...
--*/
{
    ...
    // Check for invalid parameters.
    //
    if (x >= 64 || y >= 128 || h > 1 || lpMap == NULL) { return TRUE; }
...
--*/
{
    ...
    // Check for invalid parameters.
    //
    if (y >= 128 || x >= 64 || h > 1 || lpMap == NULL) { return 0; }
