...
--*/
{
    ...
    {
        LPCBITMAPRLE lpBitmap = PAL_SpriteGetFrame(PAL_GetPlayerSprite((BYTE)i), gpGlobals->rgParty[i].wFrame);

        if (lpBitmap == NULL) { continue; }
         ...
         i++)
    {
        ...
        if (lpEvtObj->sState == kObjStateHidden || lpEvtObj->sVanishTime > 0 || lpEvtObj->sState < 0) { continue; }
...
--*/
{
    ...
    if (PAL_X(pos) < 0 || PAL_X(pos) >= 2048 || PAL_Y(pos) < 0 || PAL_Y(pos) >= 2048) { return TRUE; }
    ...
    if (xr + yr * 2 >= 16)
    {
        ...
        else if (32 - xr + yr * 2 < 16) { x++; }
        else if (32 - xr + yr * 2 < 48) { h = 1; }
        else { y++; }
    }

    if (PAL_MapTileIsBlocked(x, y, h, PAL_GetCurrentMap())) { return TRUE; }
    ...
    if (fCheckEventObjects)
    {
             ...
             i++)
        {
            ...
            if (p->sState >= kObjStateBlocker)
            {
                ...
                // Check for collision
                //
                if (abs(p->x - PAL_X(pos)) + abs(p->y - PAL_Y(pos)) * 2 < 16) { return TRUE; }
            }
        }
    }
...
--*/
{
    ...
    if (fWalking)
    {
        ...
        else { gpGlobals->rgParty[0].wFrame = gpGlobals->wPartyDirection * 3 + iStepFrameLeader; }
        ...
        for (i = 1; i <= (short)gpGlobals->wMaxPartyMemberIndex; i++)
        {
            ...
            if (i == 2)
            {
                gpGlobals->rgParty[i].x +=
                    (gpGlobals->rgTrail[1].wDirection == kDirEast || gpGlobals->rgTrail[1].wDirection == kDirWest) ? -16
                                                                                                                   : 16;
            ...
            {
                gpGlobals->rgParty[i].x +=
                    ((gpGlobals->rgTrail[1].wDirection == kDirWest || gpGlobals->rgTrail[1].wDirection == kDirSouth)
                            ? 16
                            : -16);
                gpGlobals->rgParty[i].y +=
                    ((gpGlobals->rgTrail[1].wDirection == kDirWest || gpGlobals->rgTrail[1].wDirection == kDirNorth)
                            ? 8
                            : -8);
            }
            ...
            else { gpGlobals->rgParty[i].wFrame = gpGlobals->rgTrail[2].wDirection * 3 + iStepFrameLeader; }
        }
...
--*/
{
    ...
    //
    if (wEventObjectID == 0 || wEventObjectID > gpGlobals->g.nEventObject) { return; }
