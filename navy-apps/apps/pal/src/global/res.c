...
--*/
{
    ...
    {
        for (i = 0; i < gpResources->nEventObject; i++) { free(gpResources->lppEventObjectSprites[i]); }
...
--*/
{
    ...
    if (gpResources == NULL || gpResources->bLoadFlags == 0) { return; }
    ...
    if (gpResources->bLoadFlags & kLoadScene)
    {
        ...
        for (i = 0; i < gpResources->nEventObject; i++, index++)
        {
            ...
            if (PAL_MKFDecompressChunk(gpResources->lppEventObjectSprites[i], l, n, gpGlobals->f.fpMGO) > 0)
            {
...
--*/
{
    if (gpResources == NULL) { return NULL; }
...
--*/
{
    if (gpResources == NULL || bPlayerIndex > MAX_PLAYERS_IN_PARTY) { return NULL; }
...
--*/
{
    ...
    if (gpResources == NULL || wEventObjectID >= gpResources->nEventObject) { return NULL; }
