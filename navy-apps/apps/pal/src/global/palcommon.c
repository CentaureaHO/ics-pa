...
--*/
{
    ...
    {
        T = *lpBitmapRLE++;
        if ((T & 0x80) && T <= 0x80 + uiWidth) { i += T - 0x80; }
        else
        {
            for (j = 0; j < T; j++)
            {
    ...
    for (i = 0; i < uiLen;)
    {
        ...
        else
        {
            for (j = 0; j < T; j++)
            {
                ...
                ((LPBYTE)lpDstSurface->pixels)[y * lpDstSurface->pitch + x] = (b | (lpBitmapRLE[j] & 0xF0));
            }
            lpBitmapRLE += T;
            i += T;
        }
...
--*/
{
    ...
    if (lpBitmapFBP == NULL || lpDstSurface == NULL || lpDstSurface->w != 320 || lpDstSurface->h != 200) { return -1; }
...
--*/
{
    ...
    if (lpBuffer == NULL || fp == NULL || uiBufferSize == 0) { return -1; }
