...
--*/
{
    ...
    if (fTrigger)
    {
        ...
        for (wEventObjectID = 0; wEventObjectID < gpGlobals->g.nEventObject; wEventObjectID++)
        {
            ...
            if (p->sVanishTime != 0) { p->sVanishTime += ((p->sVanishTime < 0) ? 1 : -1); }
        }
            ...
            else if (p->sState > 0 && p->wTriggerMode >= kTriggerTouchNear)
            {
                    ...
                    (p->wTriggerMode - kTriggerTouchNear) * 32 + 16)
                {
                    ...
                    if (p->nSpriteFrames)
                    {
                        ...
                        if (xOffset > 0) { p->wDirection = ((yOffset > 0) ? kDirEast : kDirNorth); }
                        else { p->wDirection = ((yOffset > 0) ? kDirSouth : kDirWest); }
                ...
                12)
        {
            ...
            for (i = 0; i < 4; i++)
            {
                ...
                if (!PAL_CheckObstacle(pos, TRUE, 0))
                {
                    ...
                    //
                    gpGlobals->viewport =
                        PAL_XY(PAL_X(pos) - PAL_X(gpGlobals->partyoffset), PAL_Y(pos) - PAL_Y(gpGlobals->partyoffset));
    ...
    while (TRUE)
    {
        ...
        if (!(gpGlobals->g.rgObject[wObject].item.wFlags & kItemFlagApplyToAll))
        {
            ...
            while (TRUE)
            {
                ...
                if (wPlayer == MENUITEM_VALUE_CANCELLED) { break; }
...
--*/
{
    ...
    if (gpGlobals->wPartyDirection == kDirNorth || gpGlobals->wPartyDirection == kDirEast) { xOffset = 16; }
    else { xOffset = -16; }

    if (gpGlobals->wPartyDirection == kDirEast || gpGlobals->wPartyDirection == kDirSouth) { yOffset = 8; }
    else { yOffset = -8; }
...
--*/
{
    ...
    if (--gpGlobals->wChasespeedChangeCycles == 0) { gpGlobals->wChaseRange = 1; }
}
...
--*/
{
    ...
    while (wTimeOut == 0 || SDL_GetTicks() < dwTimeOut)
    {
        ...
        if (g_InputState.dwKeyPress & (kKeySearch | kKeyMenu)) { break; }
    }
}
