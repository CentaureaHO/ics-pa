...
--*/
{
    ...
    if (yOffset < 0) { pEvtObj->wDirection = ((xOffset < 0) ? kDirWest : kDirNorth); }
    else { pEvtObj->wDirection = ((xOffset < 0) ? kDirSouth : kDirEast); }
    ...
    else { PAL_NPCWalkOneStep(wEventObjectID, iSpeed); }
...
--*/
{
    ...
    while (xOffset != 0 || yOffset != 0)
    {
        ...
        if (yOffset < 0) { gpGlobals->wPartyDirection = ((xOffset < 0) ? kDirWest : kDirNorth); }
        else { gpGlobals->wPartyDirection = ((xOffset < 0) ? kDirSouth : kDirEast); }

        dx = PAL_X(gpGlobals->viewport);
        dy = PAL_Y(gpGlobals->viewport);

        if (abs(xOffset) <= iSpeed * 2) { dx += xOffset; }
        else { dx += iSpeed * (xOffset < 0 ? -2 : 2); }

        if (abs(yOffset) <= iSpeed) { dy += yOffset; }
        else { dy += iSpeed * (yOffset < 0 ? -1 : 1); }
...
--*/
{
    ...
    while (xOffset != 0 || yOffset != 0)
    {
        ...
        if (abs(xOffset) > iSpeed * 2) { dx = iSpeed * (xOffset < 0 ? -2 : 2); }
        else { dx = xOffset; }

        if (abs(yOffset) > iSpeed) { dy = iSpeed * (yOffset < 0 ? -1 : 1); }
        else { dy = yOffset; }
...
--*/
{
    ...
    if (gpGlobals->wChaseRange != 0)
    {
        ...
        if (x == 0) { x = RandomLong(0, 1) ? -1 : 1; }

        if (y == 0) { y = RandomLong(0, 1) ? -1 : 1; }
        ...
        if (i + j * 2 >= 16)
        {
            ...
            else if (32 - i + j * 2 < 16) { prevx++; }
            else if (32 - i + j * 2 < 48) { l = 1; }
            else { prevy++; }
        }
        ...
        if (abs(x) + abs(y) * 2 < wChaseRange * 32 * gpGlobals->wChaseRange)
        {
            if (x < 0)
            {
                if (y < 0) { pEvtObj->wDirection = kDirWest; }
                else { pEvtObj->wDirection = kDirSouth; }
            }
            else
            {
                if (y < 0) { pEvtObj->wDirection = kDirNorth; }
                else { pEvtObj->wDirection = kDirEast; }
            }

            if (x != 0) { x = pEvtObj->x + x / abs(x) * 16; }
            else { x = pEvtObj->x; }

            if (y != 0) { y = pEvtObj->y + y / abs(y) * 8; }
            else { y = pEvtObj->y; }

            if (fFloating) { wMonsterSpeed = wSpeed; }
            else
            {
                if (!PAL_CheckObstacle(PAL_XY(x, y), TRUE, wEventObjectID)) { wMonsterSpeed = wSpeed; }
                else
                {
                    pEvtObj->x = prevx;
                    pEvtObj->y = prevy;
...
--*/
{
    LPCSTR rgszStrings[] = {"SDLPAL (http://sdlpal.codeplex.com/)",
#ifdef PAL_CLASSIC
...
--*/
{
    ...
    if (wEventObjectID != 0) { pEvtObj = &(gpGlobals->g.lprgEventObject[wEventObjectID - 1]); }
    else { pEvtObj = NULL; }
    ...
    switch (pScript->wOperation)
    {
        ...
        case 0x000F:
            ...
            //
            if (pScript->rgwOperand[0] != 0xFFFF) { pEvtObj->wDirection = pScript->rgwOperand[0]; }
            if (pScript->rgwOperand[1] != 0xFFFF) { pEvtObj->wCurrentFrameNum = pScript->rgwOperand[1]; }
            break;
        ...
        case 0x0010:
            ...
            //
            if (!PAL_NPCWalkTo(
                    wEventObjectID, pScript->rgwOperand[0], pScript->rgwOperand[1], pScript->rgwOperand[2], 3))
            {
                wScriptEntry--;
            }
            break;
        ...
        case 0x0011:
            ...
            {
                if (!PAL_NPCWalkTo(
                        wEventObjectID, pScript->rgwOperand[0], pScript->rgwOperand[1], pScript->rgwOperand[2], 2))
                {
                    wScriptEntry--;
                }
            }
        ...
        case 0x0012:
            ...
            //
            pCurrent->x = pScript->rgwOperand[1] + PAL_X(gpGlobals->viewport) + PAL_X(gpGlobals->partyoffset);
            pCurrent->y = pScript->rgwOperand[2] + PAL_Y(gpGlobals->viewport) + PAL_Y(gpGlobals->partyoffset);
            break;
        ...
        case 0x0015:
            ...
            gpGlobals->rgParty[pScript->rgwOperand[2]].wFrame = gpGlobals->wPartyDirection * 3 + pScript->rgwOperand[1];
            break;
        ...
        case 0x0018:
            ...
            if (gpGlobals->g.PlayerRoles.rgwEquipment[i][wEventObjectID] != pScript->rgwOperand[1])
            {
                ...
                if (w != 0) { PAL_AddItemToInventory(w, 1); }
        ...
        case 0x0019:
            ...
            //
            {
                ...
                if (pScript->rgwOperand[2] == 0) { iPlayerRole = wEventObjectID; }
                else { iPlayerRole = pScript->rgwOperand[2] - 1; }
        ...
        case 0x001B:
            ...
            else
            {
                ...
                //
                if (!PAL_IncreaseHPMP(wEventObjectID, (SHORT)(pScript->rgwOperand[1]), 0)) { g_fScriptSuccess = FALSE; }
            }
            break;
        ...
        case 0x001C:
            ...
            else
            {
                ...
                // Apply to one player. The wEventObjectID parameter should indicate the player role.
                //
                if (!PAL_IncreaseHPMP(wEventObjectID, 0, (SHORT)(pScript->rgwOperand[1]))) { g_fScriptSuccess = FALSE; }
            }
            break;
        ...
        case 0x001E:
            ...
            //
            if ((SHORT)(pScript->rgwOperand[0]) < 0 && gpGlobals->dwCash < (WORD)(-(SHORT)(pScript->rgwOperand[0])))
            {
                //
                // not enough cash
                //
            ...
            else { gpGlobals->dwCash += (SHORT)(pScript->rgwOperand[0]); }
            break;
        ...
        case 0x0020:
            ...
            if (!PAL_AddItemToInventory(
                    pScript->rgwOperand[0], -((pScript->rgwOperand[1] == 0) ? 1 : pScript->rgwOperand[1])))
            {
                //
                ...
                if (x > 0 && pScript->rgwOperand[2] != 0) { wScriptEntry = pScript->rgwOperand[2] - 1; }
            }
            break;
        ...
        case 0x0021:
            ...
            if (pScript->rgwOperand[0])
            {
                ...
                {
                    if (g_Battle.rgEnemy[i].wObjectID != 0) { g_Battle.rgEnemy[i].e.wHealth -= pScript->rgwOperand[1]; }
                }
            }
            else
            {
        ...
        case 0x0022:
            ...
            if (pScript->rgwOperand[0])
            {
                ...
                for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
                {
                    ...
                    if (gpGlobals->g.PlayerRoles.rgwHP[w] == 0)
                    {
                        ...
                        for (x = 0; x < kStatusAll; x++) { PAL_RemovePlayerStatus(w, x); }
            ...
            else
            {
                ...
                if (gpGlobals->g.PlayerRoles.rgwHP[wEventObjectID] == 0)
                {
                    ...
                    for (x = 0; x < kStatusAll; x++) { PAL_RemovePlayerStatus(wEventObjectID, x); }
                }
                else { g_fScriptSuccess = FALSE; }
            }
            break;
        ...
        case 0x0024:
            ...
            //
            if (pScript->rgwOperand[0] != 0) { pCurrent->wAutoScript = pScript->rgwOperand[1]; }
            break;
        ...
        case 0x0025:
            ...
            //
            if (pScript->rgwOperand[0] != 0) { pCurrent->wTriggerScript = pScript->rgwOperand[1]; }
            break;
            ...
            if (pScript->rgwOperand[0])
            {
                ...
                for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++)
                {
                    ...
                    if (RandomLong(0, 9) >= gpGlobals->g.rgObject[w].enemy.wResistanceToSorcery)
                    {
                        ...
                        {
                            if (g_Battle.rgEnemy[i].rgPoisons[j].wPoisonID == pScript->rgwOperand[1]) { break; }
                        }
                        ...
                        for (j = 0; j < MAX_POISONS; j++)
                        {
                            if (g_Battle.rgEnemy[wEventObjectID].rgPoisons[j].wPoisonID == 0)
                            {
                                ...
                                g_Battle.rgEnemy[wEventObjectID].rgPoisons[j].wPoisonScript = PAL_RunTriggerScript(
                                    gpGlobals->g.rgObject[pScript->rgwOperand[1]].poison.wEnemyScript, wEventObjectID);
                                break;
                            }
                        }
                    }
        ...
        case 0x002B:
            ...
            else { PAL_CurePoisonByKind(wEventObjectID, pScript->rgwOperand[1]); }
            break;
        ...
        case 0x002C:
            ...
            else { PAL_CurePoisonByLevel(wEventObjectID, pScript->rgwOperand[1]); }
            break;
        ...
        case 0x002E:
            ...
            else { wScriptEntry = pScript->rgwOperand[2] - 1; }
            break;
        ...
        case 0x0030:
            ...
            //
            {
                ...
                p[pScript->rgwOperand[0] * MAX_PLAYER_ROLES + iPlayerRole] =
                    p1[pScript->rgwOperand[0] * MAX_PLAYER_ROLES + iPlayerRole] * (SHORT)pScript->rgwOperand[1] / 100;
            }
            break;
        ...
        case 0x0031:
            ...
            //
            gpGlobals->rgEquipmentEffect[kBodyPartExtra].rgwSpriteNumInBattle[wEventObjectID] = pScript->rgwOperand[0];
            break;
        ...
        case 0x0033:
            ...
            else { wScriptEntry = pScript->rgwOperand[0] - 1; }
            break;
...
#else
                i = RandomLong(1, 9);
                if (i > gpGlobals->wCollectValue) { i = gpGlobals->wCollectValue; }
#endif
        ...
        case 0x0035:
            ...
            if (!pScript->rgwOperand[0]) { VIDEO_UpdateScreen(NULL); }
            break;
        ...
        case 0x0038:
            ...
            //
            if (!gpGlobals->fInBattle && gpGlobals->g.rgScene[gpGlobals->wNumScene - 1].wScriptOnTeleport != 0)
            {
        ...
        case 0x0040:
            ...
            //
            if (pScript->rgwOperand[0] != 0) { pCurrent->wTriggerMode = pScript->rgwOperand[1]; }
            break;
        ...
        case 0x0044:
            ...
            //
            PAL_PartyRideEventObject(
                wEventObjectID, pScript->rgwOperand[0], pScript->rgwOperand[1], pScript->rgwOperand[2], 4);
            break;
        ...
        case 0x0046:
            ...
            //
            {
                ...
                xOffset =
                    ((gpGlobals->wPartyDirection == kDirWest || gpGlobals->wPartyDirection == kDirSouth) ? 16 : -16);
                yOffset =
                    ((gpGlobals->wPartyDirection == kDirWest || gpGlobals->wPartyDirection == kDirNorth) ? 8 : -8);
        ...
        case 0x005B:
            ...
            if (w > pScript->rgwOperand[0]) { w = pScript->rgwOperand[0]; }
        ...
        case 0x005E:
            ...
            {
                if (g_Battle.rgEnemy[wEventObjectID].rgPoisons[i].wPoisonID == pScript->rgwOperand[0]) { break; }
            }

            if (i >= MAX_POISONS) { wScriptEntry = pScript->rgwOperand[1] - 1; }
            break;
        ...
        case 0x0061:
            ...
            //
            if (!PAL_IsPlayerPoisonedByLevel(wEventObjectID, 1)) { wScriptEntry = pScript->rgwOperand[0] - 1; }
            break;
        ...
        case 0x0066:
            ...
            w +=
                gpGlobals->g.PlayerRoles.rgwAttackStrength[gpGlobals->rgParty[g_Battle.wMovingPlayerIndex].wPlayerRole];
            w += RandomLong(0, 4);
        ...
        case 0x0068:
            ...
            //
            if (g_Battle.fEnemyMoving) { wScriptEntry = pScript->rgwOperand[0] - 1; }
            break;
        ...
        case 0x006D:
            ...
            if (pScript->rgwOperand[0])
            {
                ...
                {
                    gpGlobals->g.rgScene[pScript->rgwOperand[0] - 1].wScriptOnTeleport = pScript->rgwOperand[2];
                }
        ...
        case 0x006E:
            ...
            if (pScript->rgwOperand[0] != 0 || pScript->rgwOperand[1] != 0) { PAL_UpdatePartyGestures(TRUE); }
            break;
        ...
        case 0x0077:
            ...
            //
            PAL_PlayMUS(0, FALSE, (pScript->rgwOperand[0] == 0) ? 2.0f : (FLOAT)(pScript->rgwOperand[0]) * 2);
        ...
        case 0x0079:
            ...
            for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
            {
                if (gpGlobals->g.PlayerRoles.rgwName[gpGlobals->rgParty[i].wPlayerRole] == pScript->rgwOperand[0])
                {
                    wScriptEntry = pScript->rgwOperand[1] - 1;
                    break;
                }
        ...
        case 0x007F:
            ...
            if (pScript->rgwOperand[0] == 0 && pScript->rgwOperand[1] == 0)
            {
                ...
                gpGlobals->viewport    = PAL_XY(PAL_X(gpGlobals->viewport) + x, PAL_Y(gpGlobals->viewport) + y);
            ...
            else
            {
                ...
                do {
                    ...
                    if (pScript->rgwOperand[2] != 0xFFFF) { PAL_GameUpdate(FALSE); }
        ...
        case 0x0081:
            ...
            //
            {
                ...
                x += ((gpGlobals->wPartyDirection == kDirWest || gpGlobals->wPartyDirection == kDirSouth) ? 16 : -16);
                y += ((gpGlobals->wPartyDirection == kDirWest || gpGlobals->wPartyDirection == kDirNorth) ? 8 : -8);
        ...
        case 0x0082:
            ...
            //
            if (!PAL_NPCWalkTo(
                    wEventObjectID, pScript->rgwOperand[0], pScript->rgwOperand[1], pScript->rgwOperand[2], 8))
            {
                wScriptEntry--;
            }
            break;
        ...
        case 0x0086:
            ...
            if (!y) { wScriptEntry = pScript->rgwOperand[2] - 1; }
            break;
        ...
        case 0x008B:
            ...
            if (!gpGlobals->fNeedToFadeIn) { PAL_SetPalette(gpGlobals->wNumPalette, FALSE); }
            break;
        ...
        case 0x0090:
            ...
            //
            gpGlobals->g.rgObject[pScript->rgwOperand[0]].rgwData[2 + pScript->rgwOperand[2]] = pScript->rgwOperand[1];
            break;
        ...
        case 0x0092:
            ...
            if (gpGlobals->fInBattle)
            {
                ...
                for (i = 0; i < 5; i++)
                {
                    for (j = 0; j <= gpGlobals->wMaxPartyMemberIndex; j++) { g_Battle.rgPlayer[j].iColorShift = i * 2; }
        ...
        case 0x0093:
            ...
            //
            PAL_SceneFade(gpGlobals->wNumPalette, gpGlobals->fNightPalette, (SHORT)(pScript->rgwOperand[0]));
        ...
        case 0x0094:
            ...
            //
            if (pCurrent->sState == (SHORT)(pScript->rgwOperand[1])) { wScriptEntry = pScript->rgwOperand[2] - 1; }
            break;
        ...
        case 0x0095:
            ...
            //
            if (gpGlobals->wNumScene == pScript->rgwOperand[0]) { wScriptEntry = pScript->rgwOperand[1] - 1; }
            break;
        ...
        case 0x0098:
            ...
            if (pScript->rgwOperand[0] > 0)
            {
                ...
                gpGlobals->rgParty[gpGlobals->wMaxPartyMemberIndex + 1].wFrame = gpGlobals->rgTrail[3].wDirection * 3;
            }
            else { gpGlobals->nFollower = 0; }
            break;
        ...
        case 0x0099:
            ...
            else { gpGlobals->g.rgScene[pScript->rgwOperand[0] - 1].wMapNum = pScript->rgwOperand[1]; }
            break;
        ...
        case 0x009C:
            ...
            for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++)
            {
                if (g_Battle.rgEnemy[i].wObjectID != 0) { w++; }
            }
            ...
            if (w != 1)
            {
                ...
                //
                if (pScript->rgwOperand[1] != 0) { wScriptEntry = pScript->rgwOperand[1] - 1; }
                break;
            }
        ...
        case 0x009E:
            ...
            if (w == 0 || w == 0xFFFF) { w = g_Battle.rgEnemy[wEventObjectID].wObjectID; }

            for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++)
            {
                if (g_Battle.rgEnemy[i].wObjectID == 0) { x++; }
            }

            if (x < y || g_Battle.iHidingTime > 0 || g_Battle.rgEnemy[wEventObjectID].rgwStatus[kStatusSleep] != 0 ||
                g_Battle.rgEnemy[wEventObjectID].rgwStatus[kStatusParalyzed] != 0 ||
                g_Battle.rgEnemy[wEventObjectID].rgwStatus[kStatusConfused] != 0)
            {
                if (pScript->rgwOperand[2] != 0) { wScriptEntry = pScript->rgwOperand[2] - 1; }
            }
            else
            {
                for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++)
        ...
        case 0x009F:
            ...
            //
            if (g_Battle.iHidingTime <= 0 && g_Battle.rgEnemy[wEventObjectID].rgwStatus[kStatusSleep] == 0 &&
                g_Battle.rgEnemy[wEventObjectID].rgwStatus[kStatusParalyzed] == 0 &&
                g_Battle.rgEnemy[wEventObjectID].rgwStatus[kStatusConfused] == 0)
            {
        ...
        case 0x00A3:
            ...
            //
            if (!SOUND_PlayCDA(pScript->rgwOperand[0])) { PAL_PlayMUS(pScript->rgwOperand[1], TRUE, 0); }
            break;
        ...
        case 0x00A4:
            ...
            else { PAL_ScrollFBP(pScript->rgwOperand[0], pScript->rgwOperand[2], pScript->rgwOperand[1]); }
#endif
            break;
        ...
        case 0x00A5:
            ...
            //
#ifndef PAL_WIN95
            if (pScript->rgwOperand[1] != 0xFFFF) { PAL_EndingSetEffectSprite(pScript->rgwOperand[1]); }
...
--*/
{
    ...
    if (wEventObjectID == 0xFFFF) { wEventObjectID = wLastEventObject; }
        ...
        switch (pScript->wOperation)
        {
            ...
            case 0x0004:
                ...
                PAL_RunTriggerScript(
                    pScript->rgwOperand[0], ((pScript->rgwOperand[1] == 0) ? wEventObjectID : pScript->rgwOperand[1]));
                wScriptEntry++;
                break;
            ...
            case 0x0005:
                ...
                if (PAL_DialogIsPlayingRNG()) { VIDEO_RestoreScreen(); }
                ...
                {
                    if (pScript->rgwOperand[2]) { PAL_UpdatePartyGestures(FALSE); }
            ...
            case 0x0007:
                ...
                if (i == kBattleResultLost && pScript->rgwOperand[1] != 0) { wScriptEntry = pScript->rgwOperand[1]; }
            ...
            case 0x000A:
                ...
                if (!PAL_ConfirmMenu()) { wScriptEntry = pScript->rgwOperand[0]; }
                else { wScriptEntry++; }
                break;
            ...
            case 0x003B:
                ...
                //
                PAL_ClearDialog(TRUE);
                PAL_StartDialog(kDialogCenter, (BYTE)pScript->rgwOperand[0], 0, pScript->rgwOperand[2] ? TRUE : FALSE);
                wScriptEntry++;
                break;
            ...
            case 0x003C:
                ...
                //
                PAL_ClearDialog(TRUE);
                PAL_StartDialog(kDialogUpper,
                    (BYTE)pScript->rgwOperand[1],
                    pScript->rgwOperand[0],
...
begin:
    ...
    switch (pScript->wOperation)
    {
        ...
        case 0x0004:
            ...
            //
            PAL_RunTriggerScript(
                pScript->rgwOperand[0], pScript->rgwOperand[1] ? pScript->rgwOperand[1] : wEventObjectID);
            wScriptEntry++;
            break;
        ...
        case 0xFFFF:
            ...
            {
                PAL_DrawText(
                    PAL_GetMsg(pScript->rgwOperand[0]), PAL_XY(75, iDescLine * 16 + 150), DESCTEXT_COLOR, TRUE, FALSE);
            }
            else
            {
                PAL_DrawText(
                    PAL_GetMsg(pScript->rgwOperand[0]), PAL_XY(100, iDescLine * 16 + 3), DESCTEXT_COLOR, TRUE, FALSE);
            }
            iDescLine++;
#endif
            wScriptEntry++;
