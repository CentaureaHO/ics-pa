...
inline byte ppu_ram_read(word address) { return PPU_RAM[ppu_get_real_ram_address(address)]; }

inline void ppu_ram_write(word address, byte data) { PPU_RAM[ppu_get_real_ram_address(address)] = data; }
...
void ppu_draw_sprite_scanline()
{
    ...
    for (n = 0; n < 0x100; n += 4)
    {
        ...
        {
            int color =
                hflip ? ((((h >> x) & 1) << 1) | ((l >> x) & 1)) : ((((h >> (7 - x)) & 1) << 1) | ((l >> (7 - x)) & 1));
