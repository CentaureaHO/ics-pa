...
// Running a benchmark
static void bench_prepare(Result* res) { res->msec = _uptime(); }

static void bench_done(Result* res) { res->msec = _uptime() - res->msec; }

static const char* bench_check(Benchmark* bench)
{
    unsigned long freesp = (unsigned long)_heap.end - (unsigned long)_heap.start;
    if (freesp < setting->mlim) { return "(insufficient memory)"; }
    return NULL;
}
...
int main()
{
    ...
    for (int i = 0; i < ARR_SIZE(benchmarks); i++)
    {
        ...
        else
        {
            ...
            printk("\n");
            if (SETTING != 0) { printk("  min time: %d ms [%d]\n", (unsigned int)msec, (unsigned int)cur); }
...
{
    if ((uintptr_t)start % 16 != 0) { start = start + 16 - ((uintptr_t)start % 16); }
    char* old = start;
    start += size;
...
void bench_reset() { start = (char*)_heap.start; }

static int32_t seed = 1;

void bench_srand(int32_t _seed) { seed = _seed & 0x7fff; }
