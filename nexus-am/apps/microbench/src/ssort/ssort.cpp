...
void suffixArray(int* s, int* SA, int n, int K)
{
    ...
    int* s12 = (int*)bench_alloc(sizeof(int) * (n02 + 3));
    s12[n02] = s12[n02 + 1] = s12[n02 + 2] = 0;
    int* SA12                              = (int*)bench_alloc(sizeof(int) * (n02 + 3));
...
void bench_ssort_prepare()
{
    ...
    for (int i = 0; i < N; i++) { s[i] = bench_rand() % 26; }
}

void bench_ssort_run() { suffixArray(s, sa, N, 26); }

int bench_ssort_validate() { return checksum(sa, sa + N) == setting->checksum; }
}
