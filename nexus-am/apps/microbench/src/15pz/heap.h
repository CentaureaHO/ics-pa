...
void Updatable_heap<T, M>::percolate_down()
{
    ...
    while (2 * n + 1 <= size())
    {
        if (heap[n]->weight() < heap[2 * n]->weight() && heap[n]->weight() < heap[2 * n + 1]->weight()) { return; }
    ...
    if (2 * n == size() && heap[2 * n]->weight() < heap[n]->weight()) { swap(n, 2 * n); }
}
