...
N_puzzle<N>::N_puzzle(int array[N * N]) : puzzle_valid(true), manhattan_distance(0)
{
    ...
    for (int i = 0; i < N * N; ++i) { check[i] = false; }
...
void N_puzzle<N>::determine_hash()
{
    ...
    for (int i = 0; i < N; ++i)
    {
        for (int j = 0; j < N; ++j) { hash_value = hash_value * 1973 + puzzle[i][j]; }
    }
}
      ...
      hash_value(pz.hash_value)
{
    for (int i = 0; i < N; ++i)
    {
        for (int j = 0; j < N; ++j) { puzzle[i][j] = pz.puzzle[i][j]; }
    }
}
...
N_puzzle<N>& N_puzzle<N>::operator=(N_puzzle const& rhs)
{
    ...
    for (int i = 0; i < N; ++i)
    {
        for (int j = 0; j < N; ++j) { puzzle[i][j] = rhs.puzzle[i][j]; }
    }
    return *this;
}
...
bool N_puzzle<N>::solvable() const
{
    ...
    for (int i = 0; i < N; ++i)
    {
        for (int j = 0; j < N; ++j)
        {
            if (puzzle[i][j] == 0) { entries[N * i + j] = N * N; }
            else { entries[N * i + j] = puzzle[i][j]; }
        }
    }
...
int N_puzzle<N>::lower_bound() const
{
    ...
    for (int i = 0; i < N; ++i)
    {
        for (int j = 0; j < N; ++j)
        {
            if (puzzle[i][j] != (count % N * N)) { ++result; }
...
{
    if (!valid() || !rhs.valid() || hash() != rhs.hash()) { return false; }

    for (int i = 0; i < N; ++i)
    {
        for (int j = 0; j < N; ++j)
        {
            if (puzzle[i][j] != rhs.puzzle[i][j]) { return false; }
        }
    }
    return true;
}
...
bool N_puzzle<N>::operator!=(N_puzzle const& rhs) const
{
    ...
    for (int i = 0; i < N; ++i)
    {
        for (int j = 0; j < N; ++j)
        {
            if (puzzle[i][j] != rhs.puzzle[i][j]) { return true; }
        }
    }
...
N_puzzle<N> N_puzzle<N>::solution()
{
    ...
    for (int i = 0; i < N * N - 1; ++i) { array[i] = i + 1; }
