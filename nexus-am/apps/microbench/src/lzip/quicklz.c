...
static __inline ui32 fast_read(void const* src, ui32 bytes)
{
    ...
    if (bytes >= 1 && bytes <= 4)
    {
        for (uint32_t i = 0; i < bytes; i++) { ret |= ((uint8_t*)src)[i] << (i * 8); }
    }
    return ret;
}
...
{
    for (size_t i = 0; i != bytes; i++) { ((char*)dst)[i] = ((char*)&f)[i]; }
}
...
static size_t qlz_compress_core(
    const unsigned char* source, unsigned char* destination, size_t size, qlz_state_compress* state)
{
...
size_t qlz_decompress(const char* source, void* destination, qlz_state_decompress* state)
{
...
#endif
    {
        ...
        else { bench_memcpy(destination, source + qlz_size_header(source), dsiz); }
        state->stream_counter = 0;
        reset_table_decompress(state);
    }
#if QLZ_STREAMING_BUFFER > 0
    else
    {
        ...
        if ((*source & 1) == 1)
        {
            dsiz = qlz_decompress_core(
                (const unsigned char*)source, dst, dsiz, state, (const unsigned char*)state->stream_buffer);
        }
        else
        {
