...
static void put(uint32_t* m, int i, int j, uint32_t data) { m[i * M + j] = data; }
...
static inline void mult(uint32_t* c, uint32_t* a, uint32_t* b)
{
    for (int i = 0; i < M; i++)
        ...
        {
            put(c, i, j, 0);
            for (int k = 0; k < M; k++) { put(c, i, j, get(c, i, j) + get(a, i, k) * get(b, k, j)); }
        }
}
...
int bench_fib_validate() { return get(ans, M - 1, M - 1) == setting->checksum; }
