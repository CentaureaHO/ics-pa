...
void bench_md5_prepare()
{
    ...
    bench_srand(1);
    str = bench_alloc(N);
    for (int i = 0; i < N; i++) { str[i] = bench_rand(); }
    digest = bench_alloc(16);
}

void bench_md5_run() { md5(str, N, digest); }

int bench_md5_validate() { return checksum(digest, digest + 16) == setting->checksum; }
