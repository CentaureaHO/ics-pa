...
void matrix_mul_const(ee_u32 N, MATRES* C, MATDAT* A, MATDAT val)
{
    ...
    for (i = 0; i < N; i++)
    {
        for (j = 0; j < N; j++) { C[i * N + j] = (MATRES)A[i * N + j] * (MATRES)val; }
    }
}
...
void matrix_mul_vect(ee_u32 N, MATRES* C, MATDAT* A, MATDAT* B)
{
    ...
    {
        C[i] = 0;
        for (j = 0; j < N; j++) { C[i] += (MATRES)A[i * N + j] * (MATRES)B[j]; }
    }
}
...
void matrix_mul_matrix(ee_u32 N, MATRES* C, MATDAT* A, MATDAT* B)
{
    ...
    for (i = 0; i < N; i++)
    {
        ...
        {
            C[i * N + j] = 0;
            for (k = 0; k < N; k++) { C[i * N + j] += (MATRES)A[i * N + k] * (MATRES)B[k * N + j]; }
        }
    }
}
