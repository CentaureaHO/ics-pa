...
/* Function: main
    ...
    4 - Iterations  : Special, if set to 0, iterations will be automatically determined such that the benchmark will run
   between 10 to 100 secs
...
MAIN_RETURN_TYPE main(int argc, char* argv[])
{
    ...
    if (results[0].execs == 0) { /* if not supplied, execute all algorithms */ results[0].execs = ALL_ALGORITHMS_MASK; }
...
#error "Please define a way to initialize a memory block."
    ...
    for (i = 0; i < MULTITHREAD; i++)
    {
        ...
        if (results[i].execs & ID_STATE) { core_init_state(results[0].size, results[i].seed1, results[i].memblock[3]); }
    }
