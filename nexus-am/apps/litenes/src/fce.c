...
int fce_load_rom(char* rom)
{
    ...
    if (memcmp(fce_rom_header->signature, "NES\x1A", 4)) { return -1; }
    ...
    if (mmc_id == 0 || mmc_id == 3)
    {
        ...
        else { mmc_copy(0x8000, blk, 0x8000); }
    }
    else { return -1; }
    ...
    for (i = 0; i < fce_rom_header->chr_block_count; i++)
    {
        ...
        if (i == 0) { ppu_copy(0x0000, blk, 0x2000); }
    }
...
void wait_for_frame()
{
    ...
    while (cur - gtime < 1000 / FPS) { cur = _uptime(); }
    gtime = cur;
}
...
static const uint32_t palette[64] = {0x808080,
    0x0000BB,
    0x3700BF,
    0x8400A6,
    0xBB006A,
...
void fce_update_screen()
{
    ...
    {
        int y1 = y * (H - 1) / h + 1;
        for (int x = pad; x < w - pad; x++) { row[x] = palette[canvas[y1][xmap[x] + 0xff]]; }
...
void xmap_init()
{
    ...
    int h   = _screen.height;
    int pad = (w - h) / 2;
    for (int x = pad; x < w - pad; x++) { xmap[x] = (x - pad) * W / h; }
}
