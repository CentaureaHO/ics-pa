...
static inline void draw(int col, int row, int idx) { canvas[row][col + 0xff] = idx; }
...
inline byte ppu_ram_read(word address) { return PPU_RAM[ppu_ram_map[address]]; }

inline void ppu_ram_write(word address, byte data) { PPU_RAM[ppu_ram_map[address]] = data; }
    ...
    for (int x = 0; x < 8; x++)
    {
        for (int h = 0; h < 256; h++)
            for (int l = 0; l < 256; l++)
            {
                ...
                if (col == 0) { XHLmask[h][l] |= int_shl(0xff, x * 8); }
            }
    }

    for (int x = 0; x < 0x4000; x++) { ppu_ram_map[x] = ppu_get_real_ram_address(x); }
}
...
void ppu_draw_background_scanline(bool mirror)
{
    ...
    word attribute_address =
        (ppu_base_nametable_address() + (mirror ? 0x400 : 0) + 0x3C0 + -1 + ((ppu.scanline >> 5) << 3));
    ...
    for (tile_x = ppu_shows_background_in_leftmost_8px() ? 0 : 1; tile_x < 32; tile_x++)
    {
        ...
        if (do_update)
        {
            ...
            for (int x = 0; x < 8; x++)
            {
                ...
                if (color != 0) { draw(scroll_base + x, ppu.scanline + 1, palette_cache[color]); }
            }
        }
...
inline void ppu_copy(word address, byte* source, int length) { memcpy(&PPU_RAM[address], source, length); }
...
inline byte ppu_io_read(word address)
{
    ...
    switch (address & 7)
    {
        ...
        case 7:
        {
            ...
            if (ppu.PPUADDR < 0x3F00) { data = ppu_latch = ppu_ram_read(ppu.PPUADDR); }
            else
            {
            ...
            if (ppu_2007_first_read) { ppu_2007_first_read = false; }
            else { ppu.PPUADDR += ppu_vram_address_increment(); }
            return data;
        }
        default: return 0xFF;
    }
...
inline void ppu_io_write(word address, byte data)
{
    ...
    switch (address)
    {
        ...
        case 7:
        {
            ...
            else { ppu_ram_write(ppu.PPUADDR, data); }
        }
    }
    ppu_latch = data;
}
