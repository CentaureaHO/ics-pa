...
static void render_triangle(const ImDrawCmd* pcmd, const ImDrawVert* a, const ImDrawVert* b, const ImDrawVert* c)
{
    ...
    for (int x = minx; x <= maxx; x++)
    {
        ...
        {
            if (x >= 0 && x < _screen.width && y >= 0 && y < _screen.height && clip.x <= x && x <= clip.z &&
                clip.y <= y && y <= clip.w)
            {
                int ns = at_left(x, y, a->pos, b->pos) + at_left(x, y, b->pos, c->pos) + at_left(x, y, c->pos, a->pos);
...
{
    for (int i = 0; i < _screen.width * _screen.height; i++) fb[i] = IM_COL32(114, 144, 154, 255);
    ...
    for (int i = 0; i < _screen.width; i++)
        for (int j = 0; j < _screen.height; j++) _draw_p(i, j, fb[i + j * _screen.width]);
...
int main()
{
    ...
    while (1)
    {
        ...
        if (frames % 5 == 1) { io.MouseDown[0] = 0; }
