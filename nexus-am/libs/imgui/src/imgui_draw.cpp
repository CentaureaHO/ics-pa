...
static const ImVec4 GNullClipRect(
    -8192.0f, -8192.0f, +8192.0f, +8192.0f);  // Large values that are easy to encode in a few bits+shift
    ...
    const char* text_end, float wrap_width, const ImVec4* cpu_fine_clip_rect)
{
    ...
    // Note: This is one of the few instance of breaking the encapsulation of ImDrawList, as we pull this from ImGui
    // state, but it is just SO useful. Might just move Font/FontSize to ImDrawList?
...
ImFont* ImFontAtlas::AddFont(const ImFontConfig* font_cfg)
{
    ...
    else
    {
        IM_ASSERT(!Fonts.empty());  // When using MergeMode make sure that a font has already been added before. You can
                                    // use ImGui::AddFontDefault() to add the default imgui font.
    }
...
ImFont* ImFontAtlas::AddFontDefault(const ImFontConfig* font_cfg_template)
{
    ...
    ImFont*     font =
        AddFontFromMemoryCompressedBase85TTF(ttf_compressed_base85, 13.0f, &font_cfg, GetGlyphRangesDefault());
    return font;
}
...
// NBM Transfer ownership of 'ttf_data' to ImFontAtlas, unless font_cfg_template->FontDataOwnedByAtlas == false. Owned
...
bool ImFontAtlas::Build()
{
    ...
    // Start packing. We need a known width for the skyline algorithm. Using a cheap heuristic here to decide of width.
    // User can override TexDesiredWidth if they wish. After packing is done, width shouldn't matter much, but some
    // API/GPU have texture size limitations and increasing width can decrease height.
    ...
    memset(buf_rects,
        0,
        total_glyph_count * sizeof(stbrp_rect));  // Unnecessary but let's clear this for the sake of sanity.
    ...
    for (int input_i = 0; input_i < ConfigData.Size; input_i++)
    {
        ...
        for (int i = 0; i < tmp.RangesCount; i++)
        {
            ...
            for (int char_idx = 0; char_idx < range.num_chars; char_idx += 1)
            {
                ...
                dst_font->MetricsTotalSurface += (int)((glyph.U1 - glyph.U0) * TexWidth + 1.99f) *
...
const ImWchar* ImFontAtlas::GetGlyphRangesJapanese()
{
    ...
    // has an implicit +1. This encoding helps us reduce the source code size.
