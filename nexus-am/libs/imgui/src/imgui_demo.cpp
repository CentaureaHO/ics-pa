...
// Do NOT remove this file from your project! It is useful reference code that you and other users will want to refer
// to. Everything in this file will be stripped out by the linker if you don't call ImGui::ShowTestWindow(). During
// development, you can call ImGui::ShowTestWindow() in your code to learn about various features of ImGui. Removing
// this file from your project is hindering your access to documentation, likely leading you to poorer usage of the
// library.
...
void ImGui::ShowTestWindow(bool* p_open)
{
    ...
    {
        ImGui::TextWrapped("This window is being created by the ShowTestWindow() function. Please refer to the code "
    ...
    if (ImGui::CollapsingHeader("Window options"))
    {
        ...
        if (ImGui::TreeNode("Logging"))
        {
            ImGui::TextWrapped("The logging API redirects all text output so you can easily capture the content of a "
                               "window or a block. Tree nodes can be automatically expanded. You can also call "
                               "ImGui::LogText() to output directly to the log without a visual output.");
            ImGui::LogButtons();
            ImGui::TreePop();
        }
    }
    ...
    if (ImGui::CollapsingHeader("Widgets"))
    {
        if (ImGui::TreeNode("Trees"))
        {
            ...
            if (ImGui::TreeNode("Advanced, with Selectable nodes"))
            {
                ...
                int node_clicked = -1;  // Temporary storage of what node we have clicked to process selection at the
                ...
                for (int i = 0; i < 6; i++)
                {
                    ...
                    {
                        // Leaf: The only reason we have a TreeNode at all is to allow selection of the leaf. Otherwise
                        // we can use BulletText() or TreeAdvanceToLabelPos()+Text().
                ...
                if (node_clicked != -1)
                {
                    ...
                    else  // if (!(selection_mask & (1 << node_clicked))) // Depending on selection behavior you want,
                          // this commented bit preserve selection when clicking on item that is part of the selection
        ...
        if (ImGui::TreeNode("UTF-8 Text"))
        {
            ...
            // http://mplus-fonts.sourceforge.jp/mplus-outline-fonts/index-en.html) Most compiler appears to support
            // UTF-8 in source code (with Visual Studio you need to save your file as 'UTF-8 without signature') However
            // for the sake for maximum portability here we are *not* including raw UTF-8 character in this source file,
            // instead we encode the string with hexadecimal constants. In your own application be reasonable and use
            // UTF-8 in source or retrieve the data from file system! Note that characters values are preserved even if
            // the font cannot be displayed, so you can safely copy & paste garbled characters into another application.
            ImGui::TextWrapped("CJK text will only appears if the font was loaded with the appropriate CJK character "
                               "ranges. Call io.Font->LoadFromFileTTF() manually to load extra character ranges.");
        ...
        {
            ImGui::TextWrapped("Below we are displaying the font texture (which is the only texture we have access to "
                               "in this demo). Use the 'ImTextureID' type as storage to pass pointers or identifier to "
                               "your own texture data. Hover the texture for a zoomed view!");
            ...
            if (ImGui::IsItemHovered())
            {
                ...
                ImGui::Image(
                    tex_id, ImVec2(128, 128), uv0, uv1, ImColor(255, 255, 255, 255), ImColor(255, 255, 255, 128));
                ImGui::EndTooltip();
            }
        ...
        static int item = 1;
        ImGui::Combo("combo", &item, "aaaa\0bbbb\0cccc\0dddd\0eeee\0\0");  // Combo using values packed in a single
                                                                           // constant string (for really quick combo)

        const char* items[] = {"AAAA", "BBBB", "CCCC", "DDDD", "EEEE", "FFFF", "GGGG", "HHHH", "IIII", "JJJJ", "KKKK"};
        static int  item2   = -1;
        ImGui::Combo("combo scroll",
            &item2,
            items,
        ...
        if (ImGui::TreeNode("Range Widgets"))
        {
            ...
            ImGui::DragIntRange2(
                "range int (no bounds)", &begin_i, &end_i, 5, 0, 0, "Min: %.0f units", "Max: %.0f units");
    ...
    if (ImGui::CollapsingHeader("Layout"))
    {
        ...
        {
            ImGui::TextWrapped("(Using ImGui::BeginGroup()/EndGroup() to layout items. BeginGroup() basically locks "
                               "the horizontal position. EndGroup() bundles the whole group so that you can use "
        ...
        {
            ImGui::TextWrapped("(This is testing the vertical alignment that occurs on text to keep it at the same "
                               "baseline as widgets. Lines only composed of text or \"small\" widgets fit in less "
            ...
            ImGui::AlignFirstTextHeightToWidgets();  // If your line starts with text, call this to align it to upcoming
                                                     // widgets.
            ...
            ImGui::AlignFirstTextHeightToWidgets();  // Vertically align text node a bit lower so it'll be vertically
        ...
        if (ImGui::TreeNode("Scrolling"))
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                ImGui::BeginChild(
                    ImGui::GetID((void*)(intptr_t)i), ImVec2(ImGui::GetWindowWidth() * 0.17f, 200.0f), true);
        ...
        if (ImGui::TreeNode("Horizontal Scrolling"))
        {
            ...
            {
                // Display random stuff (for the sake of this trivial demo we are using basic Button+SameLine. If you
                // want to create your own time line for a real application you may be better off manipulating the
                // cursor position yourself, aka using SetCursorPos/SetCursorScreenPos to position the widgets yourself.
            ...
            {
                ImGui::BeginChild(
                    "scrolling");  // Demonstrate a trick: you can use Begin to set yourself in the context of another
        ...
        if (ImGui::TreeNode("Clipping"))
        {
            ...
            ImGui::TextWrapped(
                ...
                "Otherwise we are doing coarser clipping + passing a scissor rectangle to the renderer. The system is "
                "designed to try minimizing both execution and CPU/GPU rendering cost.");
            ...
            ImGui::GetWindowDrawList()->AddRectFilled(
                pos, ImVec2(pos.x + size.x, pos.y + size.y), ImColor(90, 90, 120, 255));
    ...
    if (ImGui::CollapsingHeader("Popups & Modal windows"))
    {
        ...
        {
            ImGui::TextWrapped("When a popup is active, it inhibits interacting with windows that are behind the "
            ...
            // (If you want to show the current selection inside the Button itself, you may want to build a string using
            // the "###" operator to preserve a constant ID with a variable label)
            ...
            // to make them two different menusets. If we don't, opening any popup above and hovering our menu here
            // would open it. This is because once a menu is active, we allow to switch to a sibling menu by just
            // hovering on it, which is the desired behavior for regular menus.
            ImGui::PushID("foo");
    ...
    if (ImGui::CollapsingHeader("Columns"))
    {
        ...
        ImGui::GetTextLineHeightWithSpacing()+ImGui::GetStyle().ItemSpacing.y)); ImGui::Columns(3); ImGui::Text("ID");
        ImGui::NextColumn(); ImGui::Text("Name"); ImGui::NextColumn(); ImGui::Text("Path"); ImGui::NextColumn();
            ImGui::Columns(1);
            ImGui::Separator();
            ImGui::EndChild();
    ...
    if (ImGui::CollapsingHeader("Keyboard, Mouse & Focus"))
    {
        ...
        if (ImGui::TreeNode("Mouse cursors"))
        {
            ImGui::TextWrapped(
                ...
                "You can also set io.MouseDrawCursor to ask ImGui to render the cursor for you in software.");
...
void ImGui::ShowStyleEditor(ImGuiStyle* ref)
{
    ...
    // You can pass in a reference ImGuiStyle structure to compare to, revert to and save to (else it compares to the
    // default style)
    ...
    if (ImGui::TreeNode("Rendering"))
    {
        ...
        ImGui::PushItemWidth(100);
        ImGui::DragFloat(
            "Curve Tessellation Tolerance", &style.CurveTessellationTol, 0.02f, 0.10f, FLT_MAX, NULL, 2.0f);
        ...
        if (ImGui::Button("Copy Colors"))
        {
            ...
            for (int i = 0; i < ImGuiCol_COUNT; i++)
            {
                ...
                if (!output_only_modified ||
                    ...
                    ImGui::LogText("style.Colors[ImGuiCol_%s]%*s= ImVec4(%.2ff, %.2ff, %.2ff, %.2ff);" IM_NEWLINE,
                        name,
                        22 - (int)strlen(name),
                        "",
                        col.x,
    ...
    if (ImGui::TreeNode("Fonts", "Fonts (%d)", ImGui::GetIO().Fonts->Fonts.Size))
    {
        ...
        for (int i = 0; i < atlas->Fonts.Size; i++)
        {
            ...
            ImGui::BulletText("Font %d: \'%s\', %.2f px, %d glyphs",
                i,
            ...
            if (ImGui::TreeNode("Details"))
            {
                ...
                if (ImGui::TreeNode("Glyphs", "Glyphs (%d)", font->Glyphs.Size))
                {
                    ...
                    const ImFont::Glyph* glyph_fallback =
                        font->FallbackGlyph;  // Forcefully/dodgily make FindGlyph() return NULL on fallback, which
                                              // isn't the default behavior.
                    font->FallbackGlyph = NULL;
                    for (int base = 0; base < 0x10000; base += 256)
                    {
                        int count = 0;
                        for (int n = 0; n < 256; n++) count += font->FindGlyph((ImWchar)(base + n)) ? 1 : 0;
        ...
        ImGui::DragFloat(
            "global scale", &ImGui::GetIO().FontGlobalScale, 0.005f, 0.3f, 2.0f, "%.1f");  // scale everything
        ImGui::PopItemWidth();
...
static void ShowExampleMenuFile()
{
    ...
    {
        for (int i = 0; i < ImGuiCol_COUNT; i++) ImGui::MenuItem(ImGui::GetStyleColName((ImGuiCol)i));
        ImGui::EndMenu();
    }
...
static void ShowExampleAppConstrainedResize(bool* p_open)
{
    ...
    if (type == 5)
        ImGui::SetNextWindowSizeConstraints(
            ImVec2(0, 0), ImVec2(FLT_MAX, FLT_MAX), CustomConstraints::Step, (void*)100);  // Fixed Step
    ...
    if (ImGui::Begin("Example: Constrained Resize", p_open))
    {
        ...
        for (int i = 0; i < 10; i++) ImGui::Text("Hello, sailor! Making this line long enough for the example.");
    }
    ImGui::End();
}
...
// Read section "How can I have multiple widgets with the same label? Can I have widget without a label? (Yes). A primer
// on the purpose of labels/IDs." about ID.
...
static void ShowExampleAppCustomRendering(bool* p_open)
{
    ...
    ImGui::Separator();
    {
        ...
        // IsItemHovered() However you can draw directly and poll mouse/keyboard by yourself. You can manipulate the
        ...
        draw_list->AddRectFilledMultiColor(canvas_pos,
            ...
            ImColor(50, 50, 60));
        draw_list->AddRect(
            canvas_pos, ImVec2(canvas_pos.x + canvas_size.x, canvas_pos.y + canvas_size.y), ImColor(255, 255, 255));
...
struct ExampleAppConsole
{
    ...
    ExampleAppConsole()
    {
        ...
        Commands.push_back("CLASSIFY");  // "classify" is here to provide an example of "C"+[tab] completing to "CL" and
                                         // displaying matches.
    ...
    {
        ClearLog();
        for (int i = 0; i < History.Size; i++) kfree(History[i]);
    }
    ...
    static char* Strdup(const char* str)
    {
        ...
        void*  buff = kalloc(len);
        return (char*)memcpy(buff, (const void*)str, len);
    }

    void ClearLog()
    {
        for (int i = 0; i < Items.Size; i++) kfree(Items[i]);
        Items.clear();
        ScrollToBottom = true;
    }
...
static void ShowExampleAppLayout(bool* p_open)
{
    ...
    if (ImGui::Begin("Example: Layout", p_open, ImGuiWindowFlags_MenuBar))
    {
        ...
        ImGui::BeginGroup();
        ImGui::BeginChild(
            "item view", ImVec2(0, -ImGui::GetItemsLineHeightWithSpacing()));  // Leave room for 1 line below us
    ...
    struct funcs
    {
        static void ShowDummyObject(const char* prefix, int uid)
        {
            ImGui::PushID(uid);  // Use object uid as identifier. Most commonly you could also use the object pointer as
                                 // a base ID.
            ImGui::AlignFirstTextHeightToWidgets();  // Text and Tree nodes are less high than regular widgets, here we
...
static void ShowExampleAppLongText(bool* p_open)
{
    ...
    ImGui::Combo("Test type",
        &test_type,
        "Single call to TextUnformatted()\0Multiple calls to Text(), clipped manually\0Multiple calls to Text(), not "
        "clipped\0");
    ...
    {
        for (int i = 0; i < 1000; i++) log.append("%i The quick brown fox jumps over the lazy dog\n", lines + i);
        lines += 1000;
    }
