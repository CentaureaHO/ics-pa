...
#define GATE(type, cs, entry, dpl) \
    (GateDesc) { (uint32_t)(entry) & 0xffff, (cs), 0, 0, (type), 0, (dpl), 1, (uint32_t)(entry) >> 16 }
...
static inline void set_cr0(uint32_t cr0) { asm volatile("movl %0, %%cr0" : : "r"(cr0)); }
...
static inline void set_cr3(void* pdir) { asm volatile("movl %0, %%cr3" : : "r"(pdir)); }
