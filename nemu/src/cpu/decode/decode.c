...
static inline make_DopHelper(a)
{
    op->type = OP_TYPE_REG;
    op->reg  = R_EAX;
    if (load_val) { rtl_lr(&op->val, R_EAX, op->width); }
...
static inline make_DopHelper(r)
{
    ...
    if (load_val) { rtl_lr(&op->val, op->reg, op->width); }
...
static inline make_DopHelper(O)
{
    ...
    if (load_val) { rtl_lm(&op->val, &op->addr, op->width); }
...
make_DHelper(SI2E)
{
    ...
    if (id_dest->width == 2) { id_src->val &= 0xffff; }
}
